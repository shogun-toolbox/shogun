...
TEST(CSVFileTest, vector_int32)
{
	...
	index_t len = 512 * 512;
	SGVector<index_t> data(len);
	for (index_t i = 0; i < len; i++)
		data[i] = (index_t)rand->random(index_t(0), len);
	...
	SGVector<index_t> data_from_file;
...
TEST(CSVFileTest, matrix_int32)
{
	...
	for (index_t i = 0; i < num_rows; i++)
	{
		for (index_t j = 0; j < num_cols; j++)
			data(i, j) = (index_t)rand->random(index_t(0), num_rows);
	}
...
TEST(CSVFileTest, matrix_float64)
{
	...
	index_t num_rows = 128;
	index_t num_cols = 128;
...
TEST(CSVFileTest, string_list_char)
{
	...
	{
		for (index_t j = 0; j < lines_to_read[i].slen; j++)
		{
