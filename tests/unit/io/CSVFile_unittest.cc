...
TEST(CSVFileTest, load_test)
{
	/* This test is different from matrix_float64 as it uses the csv file
	 * generated by savetxt method in numpy (See issue #1991 for details)
	 */
	CCSVFile* cfile = new CCSVFile("test_20k.csv");
	SGMatrix<double> mat = SGMatrix<double>();
	mat.load(cfile);
	SG_UNREF(cfile);
	
	EXPECT_EQ(mat.num_cols, 20000);
	EXPECT_EQ(mat.num_rows, 70);
	//unlink("test_20k.csv");
}
