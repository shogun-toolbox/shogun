...
TEST(MaternKernelTest_1_2, test_kernel_matrix)
{
	...
	auto kernel =
	    std::make_shared<MaternKernel>(features, features, 10, 2.0, 0.5);
...
TEST(MaternKernelTest_3_2, test_kernel_matrix)
{
	...
	auto kernel =
	    std::make_shared<MaternKernel>(features, features, 10, 2.0, 1.5);
	SGMatrix<float64_t> computed_kernel_matrix = kernel->get_kernel_matrix();
...
TEST(MaternKernelTest_5_2, test_kernel_matrix)
{
	...
	auto kernel =
	    std::make_shared<MaternKernel>(features, features, 10, 2.0, 2.5);
	SGMatrix<float64_t> computed_kernel_matrix = kernel->get_kernel_matrix();
