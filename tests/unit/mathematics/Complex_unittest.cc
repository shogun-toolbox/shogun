...
TEST(CMath, complex_test)
{
	...
	result=CMath::sqrt(a);
	EXPECT_NEAR(result.real(), 2.53083481048315883655, 1E-15);
	EXPECT_NEAR(result.imag(), 1.18537961765559618499, 1E-15);
	...
	EXPECT_NEAR(result.imag(), 57.21839505634108746790, 1E-15);
	result=CMath::sinh(a);
	EXPECT_NEAR(result.real(), 71.24771797085288937978, 1E-15);
	EXPECT_NEAR(result.imag(), -20.73540973837024026238, 1E-15);

	result=CMath::cos(a);
	EXPECT_NEAR(result.real(), 57.21909818460073893220, 1E-15);
	EXPECT_NEAR(result.imag(), 193.42764312130648818311, 1E-15);
	result=CMath::cosh(a);
	EXPECT_NEAR(result.real(), 71.25418754735444792914, 1E-15);
	EXPECT_NEAR(result.imag(), -20.73352705155264885661, 1E-15);

	result=CMath::tan(a);
	EXPECT_NEAR(result.real(), -0.00000668523139027702, 1E-15);
