...
TEST(StructuredLabels, add_label)
{
	...
	l->add_label(new RealNumber(7));
	l->add_label(new RealNumber(13));

	RealNumber* real_number;

	EXPECT_EQ(3, l->get_num_labels());

	real_number = RealNumber::obtain_from_generic(l->get_label(0));
	EXPECT_EQ(3, real_number->value);
	SG_UNREF(real_number);

	real_number = RealNumber::obtain_from_generic(l->get_label(1));
	EXPECT_EQ(7, real_number->value);
	SG_UNREF(real_number);

	real_number = RealNumber::obtain_from_generic(l->get_label(2));
...
TEST(StructuredLabels, set_label)
{
	...
	l->set_label(1, new RealNumber(23));
