...
TEST(NewtonSVM, continue_training_consistency)
{
	...
	auto results = svm->predict(test_features);
	...
	svm_one->fit(features, labels);

	auto results_one = svm_one->predict(test_features);
	auto results_stop = svm_stop->predict(test_features);
	...
	auto results_complete = svm_stop->predict(test_features);
