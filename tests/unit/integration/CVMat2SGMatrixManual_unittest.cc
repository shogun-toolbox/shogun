...
TEST(CVMat2SGMatrixManual, CVMat_to_SGMatrix_conversion_using_Manual)
{
	...
	for (int l=0; l<7; l++)
    {
	    ...
	    CDenseFeatures<uint8_t>* A1 = CV2FeaturesFactory::getDenseFeatures<uint8_t>(cvMat,CV2SG_MANUAL);
	    CDenseFeatures<int8_t>* A2 = CV2FeaturesFactory::getDenseFeatures<int8_t>(cvMat,CV2SG_MANUAL);
	    CDenseFeatures<uint16_t>* A3 = CV2FeaturesFactory::getDenseFeatures<uint16_t>(cvMat,CV2SG_MANUAL);
	    CDenseFeatures<int16_t>* A4 = CV2FeaturesFactory::getDenseFeatures<int16_t>(cvMat,CV2SG_MANUAL);
	    CDenseFeatures<int32_t>* A5 = CV2FeaturesFactory::getDenseFeatures<int32_t>(cvMat,CV2SG_MANUAL);
	    CDenseFeatures<float32_t>* A6 = CV2FeaturesFactory::getDenseFeatures<float32_t>(cvMat,CV2SG_MANUAL);
	    CDenseFeatures<float64_t>* A7 = CV2FeaturesFactory::getDenseFeatures<float64_t>(cvMat,CV2SG_MANUAL);
