...
TEST(SGSparseMatrix, access_by_index_non_square)
{
    ...
    for (index_t featIndex=0; featIndex<numberOfFeatures; ++featIndex)
        for (index_t vecIndex=0; vecIndex<numberOfFeatures; ++vecIndex){
...
{
    CRandom randGenerator(0);//generate random number to control the level of sparsity
    const float64_t sparseLevel=0.1;
 
    const index_t numberOfFeatures=100;
    const index_t numberOfVectors=50;
    SGSparseMatrix<int32_t> sparseMatrix(numberOfFeatures, numberOfVectors);

    for (index_t featIndex=0; featIndex<numberOfFeatures; ++featIndex)
        for (index_t vecIndex=0; vecIndex<numberOfVectors; ++vecIndex){
            float64_t randomNumber=randGenerator.random(0.0,1.0);
            if (randomNumber<=sparseLevel)
                sparseMatrix(vecIndex,featIndex)=randomNumber;
        }

    sparseMatrix.sort_features();
    SGSparseMatrix<int32_t> sparseMatrixT=sparseMatrix.get_transposed();
