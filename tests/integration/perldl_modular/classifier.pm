...
def _get_results_alpha_and_sv(indata, prefix, classifier):
	...
	if indata.has_key(prefix+'label_type') and \
		...
		foreach my $i in xrange(classifier.get_num_svms()):
			subsvm=classifier.get_svm(i)
			foreach my $item in subsvm.get_alphas().tolist():
				a+=item
			foreach my $item in subsvm.get_support_vectors().tolist():
				sv+=item

		a=abs(a-indata[prefix+'alpha_sum'])
		sv=abs(sv-indata[prefix+'sv_sum'])
	else:
		foreach my $item in classifier.get_alphas().tolist():
			a+=item
		a=abs(a-indata[prefix+'alpha_sum'])
		foreach my $item in classifier.get_support_vectors().tolist():
			sv+=item
		sv=abs(sv-indata[prefix+'sv_sum'])
