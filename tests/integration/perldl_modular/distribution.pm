...
{my  ($indata) = @_;
 ...
 my $num_param=$distribution_>get_num_model_parameters();
 my $derivatives=0;
 foreach my $i (0..$num_param) {
     foreach my $j (0..$num_examples) {
 ...
 if( $indata->{$prefix.'name'} eq 'HMM'){
     ...
     foreach my $i (0..$indata->{$prefix.'num_examples'}) {
	 $best_path+=$distribution->best_path($i);
	 foreach my $j (0..$indata->{$prefix.'N'}) {
