...
int main()
{
	...
	DenseFeatures<float64_t>* features=new DenseFeatures<float64_t>(feat_mat);
	...
	MulticlassLabels* labels=new MulticlassLabels(lab_vec);

	// create LMNN metric machine
	int32_t k=1;	// number of target neighbors per example
	LMNN* lmnn=new LMNN(features,labels,k);
...
//	lmnn->get_linear_transform().display_matrix("linear_transform");
	LMNNStatistics* statistics=lmnn->get_statistics();
/*
