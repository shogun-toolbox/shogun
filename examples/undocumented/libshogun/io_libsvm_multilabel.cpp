...
void test_libsvmfile_multilabel()
{
	FILE * pfile = fopen(fname_svm_multilabel, "r");
	if (pfile == NULL)
	{
		SG_SPRINT("Unable to open file: %s\n", fname_svm_multilabel);
		return;
	}

	/* sparse data from matrix*/
	CLibSVMFile * svmfile = new CLibSVMFile(fname_svm_multilabel);

	SGSparseVector<float64_t>* feats;
	SGSparseVector<float64_t>* labels;
	int32_t dim_feat;
	int32_t num_samples;
	int32_t num_classes;
