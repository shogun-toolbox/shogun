...
{
    // Read training data
    SGMatrix<int32_t> labels;
    SGMatrix<float64_t> feats;
    read_data(FNAME_TRAIN, labels, feats);
	...
	CFactorGraphFeatures* fg_feats = new CFactorGraphFeatures(num_sample);
	SG_REF(fg_feats);
	CFactorGraphLabels* fg_labels = new CFactorGraphLabels(num_sample);
	SG_REF(fg_labels);
	...
	CFactorGraphModel* model = new CFactorGraphModel(fg_feats, fg_labels, TREE_MAX_PROD, false);
	SG_REF(model);
	...
	// create SGD solver
	CStochasticSOSVM* sgd = new CStochasticSOSVM(model, fg_labels);
	...
	ave_loss_sgd = acc_loss_sgd / static_cast<float64_t>(num_sample);
