...
def kernel_combined_custom_poly (train_fname = traindat,test_fname = testdat,train_label_fname=label_traindat):
    ...
    tkernel = sg.create("PolyKernel", cache_size=10, degree=3)
    ...
    subkernel = sg.create("PolyKernel", cache_size=10, degree=2)
    kernel.add("kernel_array", subkernel)

    kernel.init(feats_train, feats_train)

    labels = BinaryLabels(sg.read_csv(train_label_fname))
    svm = sg.create("LibSVM", C1=1.0, C2=1.0, kernel=kernel, labels=labels)
    svm.train()

    kernel = sg.create("CombinedKernel")
    feats_pred = sg.create("CombinedFeatures")
    ...
    feats_pred.add("feature_array", subkfeats_test)
    subkernel = sg.create("PolyKernel", cache_size=10, degree=2)
    kernel.add("kernel_array", subkernel)
