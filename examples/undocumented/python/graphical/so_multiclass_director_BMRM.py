...
class MulticlassStructuredModel(sg.DirectorStructuredModel):
	def __init__(self,features,labels):
		sg.DirectorStructuredModel.__init__(self)
	...
	def argmax(self,w,feat_idx,training):
		...
		label = None
		if training == True:
			label = int(sg.RealNumber.obtain_from_generic(self.get_labels().get_label(feat_idx)).value)
		ypred = 0
		max_score = -1e10
		...
		res = sg.ResultSet()
		res.score = max_score
		res.psi_pred = sg.RealVector(self.dim)
		res.psi_pred.zero()
		for i in xrange(self.n_feats):
			res.psi_pred[i+self.n_feats*ypred] = feature_vector[i]
		res.argmax = sg.RealNumber(ypred)
		if training == True:
			res.delta = (label!=ypred)
			res.psi_truth = sg.RealVector(self.dim)
			res.psi_truth.zero()
			for i in xrange(self.n_feats):
