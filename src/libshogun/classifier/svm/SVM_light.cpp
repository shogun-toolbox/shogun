...
#ifdef CYGWIN
  for (;((iteration<3) || (retrain && (!terminate))); iteration++){
#else
  CSignal::clear_cancel();
  for (;((!CSignal::cancel_computations()) && ((iteration<3) || (retrain && (!terminate)))); iteration++){
#endif
    ...
    // !!!
	  ...
	  /* mkl (!get_mkl_enabled()) && */
	  if ((!retrain) && (inactivenum>0) && ((!learn_parm->skip_final_opt_check) || (kernel->has_property(KP_LINADD) && get_linadd_enabled()))) { 
		  t1=get_runtime();
	  ...
	  //don't shrink w/ mkl (!get_mkl_enabled()) && 
