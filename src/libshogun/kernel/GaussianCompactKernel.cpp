...
#include "kernel/GaussianCompactKernel.h"
...
CGaussianCompactKernel::CGaussianCompactKernel(int32_t size, float64_t w)
: CDotKernel(size)
{
	init();
	set_width(w);
...
void CGaussianCompactKernel::cleanup()
{
	if (sq_lhs != sq_rhs)
		delete[] sq_rhs;
	sq_rhs = NULL;
...
void CGaussianCompactKernel::precompute_squared_helper(float64_t* &buf, CDotFeatures* df)
{
	ASSERT(df);
...
bool CGaussianCompactKernel::init(CFeatures* l, CFeatures* r)
{
...
void CGaussianCompactKernel::load_serializable_post(void) throw (ShogunException)
{
	CKernel::load_serializable_post();
	precompute_squared();
}

void CGaussianCompactKernel::precompute_squared()
{
	if (!lhs || !rhs)
		return;
...
void CGaussianCompactKernel::init()
{
	set_width(1.0);
	sq_lhs=NULL;
	sq_rhs=NULL;
