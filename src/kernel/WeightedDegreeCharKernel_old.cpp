...
#include "kernel/WeightedDegreeCharKernel_old.h"
...
CWeightedDegreeCharKernel_old::CWeightedDegreeCharKernel_old(LONG size, double* w, INT d, INT max_mismatch_, bool use_norm)
...
CWeightedDegreeCharKernel_old::~CWeightedDegreeCharKernel_old() 
{
	cleanup();
...
void CWeightedDegreeCharKernel_old::remove_lhs() 
{ 
	delete_optimization();
...
void CWeightedDegreeCharKernel_old::remove_rhs()
{
	if (rhs)
		cache_reset() ;
...
bool CWeightedDegreeCharKernel_old::init(CFeatures* l, CFeatures* r, bool do_init)
{
...
void CWeightedDegreeCharKernel_old::cleanup()
{
	delete_optimization();
...
bool CWeightedDegreeCharKernel_old::load_init(FILE* src)
{
    assert(src!=NULL);
    UINT intlen=0;
    UINT endian=0;
...
bool CWeightedDegreeCharKernel_old::save_init(FILE* dest)
{
	return false;
}
  

bool CWeightedDegreeCharKernel_old::init_optimization(INT count, INT * IDX, REAL * alphas)
{
	if (max_mismatch!=0)
	{
		CIO::message(M_ERROR, "CWeightedDegreeCharKernel_old optimization not implemented for mismatch!=0\n") ;
		return false ;
	}

	delete_optimization();
	
	CIO::message(M_DEBUG, "initializing CWeightedDegreeCharKernel_old optimization\n") ;
	int i=0;
	for (i=0; i<count; i++)
	{
		if ( (i % (count/10+1)) == 0)
...
bool CWeightedDegreeCharKernel_old::delete_optimization() 
{ 
	CIO::message(M_DEBUG, "deleting CWeightedDegreeCharKernel_old optimization\n");
...
REAL CWeightedDegreeCharKernel_old::compute_with_mismatch(CHAR* avec, INT alen, CHAR* bvec, INT blen)
{
	REAL sum = 0.0 ;
...
REAL CWeightedDegreeCharKernel_old::compute_without_mismatch(CHAR* avec, INT alen, CHAR* bvec, INT blen)
{
	REAL sum = 0.0 ;
...
REAL CWeightedDegreeCharKernel_old::compute_without_mismatch_matrix(CHAR* avec, INT alen, CHAR* bvec, INT blen)
{
	REAL sum = 0.0 ;
...
REAL CWeightedDegreeCharKernel_old::compute(INT idx_a, INT idx_b)
{
  INT alen, blen;
  bool afree, bfree;
...
void CWeightedDegreeCharKernel_old::add_example_to_tree(INT idx, REAL alpha) 
{
	INT len ;
	bool free ;
...
REAL CWeightedDegreeCharKernel_old::compute_by_tree(INT idx) 
{
	INT len ;
	bool free ;
...
void CWeightedDegreeCharKernel_old::compute_by_tree(INT idx, REAL* LevelContrib) 
{
	INT slen ;
	bool free ;
...
REAL CWeightedDegreeCharKernel_old::compute_abs_weights_tree(struct SuffixTree* p_tree) 
{
	REAL ret=0 ;
...
REAL *CWeightedDegreeCharKernel_old::compute_abs_weights(int &len) 
{
...
void CWeightedDegreeCharKernel_old::count_tree_usage(INT idx) 
{
	INT len ;
	bool free ;
	CHAR* char_vec=((CCharFeatures*) lhs)->get_feature_vector(idx, len, free);
...
void CWeightedDegreeCharKernel_old::prune_tree(struct SuffixTree * p_tree, int min_usage)
{
	if (p_tree==NULL)
	{
		if (trees==NULL)
...
void CWeightedDegreeCharKernel_old::delete_tree(struct SuffixTree * p_tree)
{
	if (p_tree==NULL)
	{
		if (trees==NULL)
...
INT CWeightedDegreeCharKernel_old::tree_size(struct SuffixTree * p_tree)
{
	INT ret=0 ;
...
bool CWeightedDegreeCharKernel_old::set_weights(REAL* ws, INT d, INT len)
{
...
bool CWeightedDegreeCharKernel_old::set_position_weights(REAL* pws, INT len)
{
	if (len==0)
	{
		delete[] position_weights ;
