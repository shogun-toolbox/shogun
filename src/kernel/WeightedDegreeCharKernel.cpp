...
inline REAL CWeightedDegreeCharKernel::compute_optimized_active_helper(INT idx, INT tree_idx) 
{
  INT len ;
  bool free ;
...
/*    if (!is_optimizable())
    ...
    return jj;
*/
	INT AA[256] ; for (INT kk=0; kk<256; kk++) AA[kk]=0 ;
	AA[(INT)'a']=0 ; AA[(INT)'A']=0 ;
	AA[(INT)'c']=1 ; AA[(INT)'C']=1 ;
	AA[(INT)'g']=2 ; AA[(INT)'G']=2 ;
	AA[(INT)'t']=3 ; AA[(INT)'T']=3 ;
	...
	INT tree_idx=0; 
	for (tree_idx=0; tree_idx<len; tree_idx++)
	{
		struct Trie *tree = trees[tree_idx] ;
		assert(tree!=NULL) ;
		for(jj=start; ((jjj=active_idx[jj])>=0); jj++) 
		{
			const INT idx = example_idx[jjj] ;
			REAL sum=0.0 ;
			bool free ;
			CHAR* vec=((CCharFeatures*) rhs)->get_feature_vector(idx, len, free);
			...
			for (INT j=0; (j<degree) && (tree_idx+j<len); j++)
			{
				if ((!tree->has_floats) && (tree->childs[AA[(INT)vec[tree_idx+j]]]!=NO_CHILD))
				{
#ifdef USE_TREEMEM
					tree = &TreeMem[tree->childs[AA[(INT)vec[tree_idx+j]]]] ;
#else
					tree = tree->childs[AA[(INT)vec[tree_idx+j]]] ;
#endif
					sum += tree->weight ;
				} else
					if (tree->has_floats)
					{
						sum += tree->child_weights[AA[(INT)vec[tree_idx+j]]] ;
						break ;
					} else
						break ;
			} 
			...
			if (use_normalization)
				active_output[jjj]+=sum/sqrtdiag_rhs[idx] ;
			else
				active_output[jjj]+=sum ;
		}
	}
	...
	return jj;
		...
		//	sum+=compute_optimized_active_helper(example_idx[j], i);
