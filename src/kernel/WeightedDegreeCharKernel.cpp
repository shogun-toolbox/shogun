...
void CWeightedDegreeCharKernel::add_example_to_single_tree(INT idx, DREAL alpha, INT tree_num) 
{
	...
	if (length == 0 || max_mismatch > 0)
	{
		...
		if (alpha_pw!=0.0)
		{
			...
			{
				if ((j<degree-1) && (tree->children[vec[tree_num+j]]!=NO_CHILD))
				{
#ifdef USE_TREEMEM
...
void CWeightedDegreeCharKernel::add_example_to_single_tree_mismatch(INT idx, DREAL alpha, INT tree_num) 
{
	...
	if (position_weights!=NULL)
		alpha_pw = alpha*position_weights[tree_num] ;
	if (alpha_pw!=0.0)
		add_example_to_tree_mismatch_recursion(tree, alpha_pw, &vec[tree_num], len-tree_num, 0, 0) ;
...
void CWeightedDegreeCharKernel::compute_by_tree(INT idx, DREAL* LevelContrib) 
{
	...
	for (INT i=0; i<len; i++)
		compute_by_tree_helper(vec, len, i, LevelContrib, factor);
	...
	for (INT j=0; j<num_feat; j++)
	{
		...
		for (INT i=0; i<num_vec; i++)
		{
			...
			if (use_normalization)
				...
				result[i] += factor*compute_by_tree_helper(vec, len, j);
...
void CWeightedDegreeCharKernel::compute_scoring_helper(struct Trie* tree, INT i, INT j, DREAL weight, INT d, INT max_degree, INT num_feat, INT num_sym, INT sym_offset, INT offs, DREAL* result)
{
	if (i+j<num_feat)
	{
		if (j<degree-1)
