...
class CWeightedDegreePositionCharKernel_old: public CCharKernel
{
 public:
  CWeightedDegreePositionCharKernel_old(LONG size, REAL* weights, INT degree, INT max_mismatch, 
									INT * shift, INT shift_len, bool use_norm=false,
									INT mkl_stepsize=1) ;
  ~CWeightedDegreePositionCharKernel_old() ;
  ...
  virtual EKernelType get_kernel_type() { return K_WEIGHTEDDEGREEPOSOLD; }

  // return the name of a kernel
  virtual const CHAR* get_name() { return "WeightedDegreePos_old" ; } ;
  ...
  virtual REAL compute_optimized(INT idx) 
	  { 
		  ...
		  CIO::message(M_ERROR, "CWeightedDegreePositionCharKernel_old optimization not initialized\n") ;
		  return 0 ;
	  } ;
...
inline REAL CWeightedDegreePositionCharKernel_old::compute_by_tree_helper(INT* vec, INT seq_pos, INT tree_pos,
																	   INT weight_pos)
{
	REAL sum=0 ;
...
/* computes the simple kernel between position seq_pos and tree tree_pos */
inline void CWeightedDegreePositionCharKernel_old::compute_by_tree_helper(INT* vec, INT seq_pos, INT tree_pos, 
																	  INT weight_pos, 
																	  REAL* LevelContrib, REAL factor) 
{
	struct SuffixTree *tree = trees[tree_pos] ;
