...
bool CSVM::svm_test(CObservation* test, FILE* outfile)
{
    ...
    if (svm.kernel_parm.kernel_type==4) // standard hmm+svm
    {
	featurespace=pos->compute_top_feature_cache(pos, neg, num_features);

	if (featurespace)
	    svm.kernel_parm.kernel_type=6; // hmm+svm precalculated
    }
...
double CSVM::classify_example(MODEL *model,DOC *ex) /* classifies example */
{
  register long i;
...
void CSVM::read_model(char *modelfile, MODEL *model,long max_words, long ll)
{
  FILE* modelfl;
  long j,i;
  char *line;
...
void CSVM::select_top_n(double *selcrit, long range,long* select, long n)
{
  register long i,j;
...
void CSVM::init_shrink_state(SHRINK_STATE *shrink_state, long totdoc, long maxhistory)
{
  long i;
...
long CSVM::shrink_problem(LEARN_PARM *learn_parm, SHRINK_STATE *shrink_state, long *active2dnum, 
...
void CSVM::cache_kernel_row(KERNEL_CACHE *kernel_cache, DOC *docs, long m, KERNEL_PARM *kernel_parm)
{
  register DOC *ex;
  register long j,k,l;
...
void CSVM::cache_multiple_kernel_rows(KERNEL_CACHE *kernel_cache, DOC *docs, long *key, long varnum, KERNEL_PARM *kernel_parm)
{
  register long i;
...
void CSVM::kernel_cache_shrink(KERNEL_CACHE *kernel_cache, long totdoc, long numshrink, long *after)
{                           
...
void CSVM::kernel_cache_init(KERNEL_CACHE *kernel_cache, long totdoc, long buffsize)
{
  long i;
...
/* element */
long CSVM::kernel_cache_free_lru(KERNEL_CACHE *kernel_cache)  
{                                     
...
/* element is removed. */
CFLOAT* CSVM::kernel_cache_clean_and_malloc(KERNEL_CACHE *kernel_cache, long docnum)
{             
  long result;
...
long CSVM::kernel_cache_touch(KERNEL_CACHE *kernel_cache, long docnum)
{
...
/* Is that row cached? */
long CSVM::kernel_cache_check(KERNEL_CACHE *kernel_cache, long docnum)
{
...
void CSVM::estimate_transduction_quality(MODEL *model, long *label, long* unlabeled, long totdoc, DOC *docs, double *lin)
{
...
double CSVM::estimate_margin_vcdim(MODEL *model, double w, double R, KERNEL_PARM *kernel_parm)
{
  double h;
...
double CSVM::estimate_sphere(MODEL *model, KERNEL_PARM *kernel_parm)
...
void CSVM::write_prediction(char *predfile, MODEL *model, double *lin, double *a, 
...
void CSVM::write_alphas(char *alphafile, double *a, long *label, long totdoc)
{
  FILE *alphafl;
  long i;
