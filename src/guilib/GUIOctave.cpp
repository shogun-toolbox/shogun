...
bool CGUIOctave::best_path_trans(const octave_value_list& vals, octave_value_list& retvals)
{
	   ...
	   )
	{
		...
		REAL* dict_weights =  new REAL[mat_dict_weights.length()];
...
bool CGUIOctave::set_plugin_estimate(const octave_value_list& vals)
{
	...
	const REAL* neg_params = &(result[num_params]) ;
...
{
	const RowVector lab=vals(2).row_vector_value();

	if ( lab.cols() )
	{
		CLabels* label=new CLabels(lab.cols());
