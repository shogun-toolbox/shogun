...
SEXP CGUI_R::get_features(CFeatures* f)
{
	if (f)
	{
      ...
      //Rprintf("feature class is %d\n", f->get_feature_class() );
		switch (f->get_feature_class())
		{
			case C_SIMPLE:
            //Rprintf("feature type is %d\n", f->get_feature_type() );
				switch (f->get_feature_type())
				{
					case F_DREAL:

						int rows = ((CRealFeatures*) f)->get_num_vectors();
                  int cols = ((CRealFeatures*) f)->get_num_features();
                  
                  //Rprintf("rows/cols %d/%d\n", rows , cols );
						...
						for (INT i=0; i<((CRealFeatures*) f)->get_num_vectors(); i++)
                  {
                     ...
                     for (INT j=0; j<num_feat; j++) {
                        //Rprintf("Value at %d/%d: %f\n", i,j, (double) vec[j]  );
                        REAL(feat)[cols*i+j] = (double) vec[j];
                     }
...
CFeatures* CGUI_R::set_features(SEXP feat)
{
   ...
   CIO::message(M_INFO, "start CGUI_R::set_features\n") ;
	...
	if (feat)
	{
      ...
      Rprintf("number of cols: %d\n", cols);
      Rprintf("typeid of feat is: %d\n", TYPEOF(feat));
#endif
		...
		else
		{
			...
			if ( TYPEOF(feat) == CHARSXP || TYPEOF(feat) == STRSXP )
		   {
           ...
           /*
				if (nrhs==4)
				{
					CHAR* al = CGUI_R::get_mxString(vals[3]);
            ...
            */
            ...
            num_feat = 4;
				...
				for (INT i=0; i<num_vec; i++)
				  for (INT j=0; j<num_feat; j++)
              {
				    //fm[i*num_feat+j]= CHAR(STRING_ELT(VECTOR_ELT(feat,i),j));
				    fm[i*num_feat+j]= CHAR(VECTOR_ELT(feat,i))[j];
				    //fm[i*num_feat+j]= STRING_ELT(feat,i*num_feat+j);
                //Rprintf("actual char %s %d\n", CHAR(feat)[i]   ,  (int) CHAR(feat)[i]  );
              }
...
//			else if (mxIsCell(feat))
//			{
//				int num_vec=mxGetNumberOfElements(feat);
//
//				ASSERT(num_vec>=1 && mxGetCell(feat, 0));
//				T_STRING<CHAR>* sc=new T_STRING<CHAR>[num_vec];
//
//				if (mxIsChar(mxGetCell(feat, 0)))
//				{
   ...
   SEXP lab ;
	...
	if (label)
	{
      ...
      for (int i=0; i< label->get_num_labels(); i++)
		   INTEGER(lab)[i]=label->get_label(i);
...
CLabels* CGUI_R::set_labels(SEXP labelsR)
{
   if(labelsR)
	{
#ifdef DEBUG
      Rprintf("lenght of labels is %d", length(labelsR));
#endif
...
SEXP CGUI_R::get_kernel_matrix()
{
	...
	if (k && k->get_rhs() && k->get_lhs())
	{
...
#ifdef DEBUG
      Rprintf("Kernel matrix has size %d / %d\n", num_vec1, num_vec2);
#endif
      SEXP ans,dim;
      ...
      if( blub == NULL )
#ifdef DEBUG
         Rprintf("error return value is NULL!");
#endif
...
SEXP CGUI_R::get_subkernel_weights()
{
   ...
   SEXP result;
	...
	if (kernel_ && (kernel_->get_kernel_type() == K_WEIGHTEDDEGREE))
	{
      ...
      //Rprintf("getting degree weights...");
	...
	{
      //Rprintf("getting combined weights...");
