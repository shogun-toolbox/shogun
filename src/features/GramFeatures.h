...
#ifndef _CGRAMFEATURES__H__
#define _CGRAMFEATURES__H__
...
// GramFeatures
class CGramFeatures: public CFeatures
{
     ...
     */
	CGramFeatures(CHAR *fname, CHAR *aname, BYTE nlen)  
	    : CFeatures(fname)
	{
     ...
     * @param nlen N-gram length
     */
	CGramFeatures(CStringFeatures<CHAR> *sf, CHAR *aname, BYTE nlen)  
	    : CFeatures("XXX")
	{
	    ASSERT(aname && nlen > 0);
     ...
     * @param aname Alphabet name, e.g. bytes, ascii, text, dna
     * @param delim Byte array of delimiters
     * @param len   Length of byte array
     */
	CGramFeatures(CHAR *fname, CHAR *aname, BYTE *delim, INT len) 
	    : CFeatures(fname)
	{
	    ASSERT(fname && aname && delim && len > 0);
	    
	    /* Allocate and generate gram configuration (words) */
        alph_type_t at = alph_get_type(aname);
	    cfg = gram_cfg_words(alph_create(at), (byte_t *) delim, len);
     ...
     * @param len   Length of byte array
     */
	CGramFeatures(CStringFeatures<CHAR> *sf, CHAR *aname, BYTE *delim, INT len) 
	    : CFeatures("XXX")
	{
	    ASSERT(aname && delim && len > 0);
     ...
     */
	CGramFeatures(const CGramFeatures &orig) 
	    : CFeatures(orig)
	{
