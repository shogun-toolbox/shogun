...
CStringFeatures::CStringFeatures(long size) : CFeatures(size), num_vectors(0), num_features(0), feature_matrix(NULL), feature_cache(NULL)
{
}
...
num_vectors(orig.num_vectors), num_features(orig.num_features)
{
	...
	{
		feature_matrix=new CHAR(num_vectors*num_features);
		memcpy(feature_matrix, orig.feature_matrix, sizeof(CHAR)*num_vectors*num_features); 
	}
}

/// get feature vector for sample num
CHAR* CStringFeatures::get_feature_vector(long num, long &len, bool &free)
{
	len=num_features; 
	...
	else
	{
		...
		if (get_num_preproc())
		{
			...
			{
				tmp_feat_after=((CStringPreProc*) get_preproc(i))->apply_to_feature_vector(tmp_feat_before, tmp_len);
...
void CStringFeatures::free_feature_vector(REAL* feat, int num, bool free)
{
	if (feature_cache)
...
REAL* CStringFeatures::get_feature_matrix(long &num_feat, long &num_vec)
{
...
bool CStringFeatures::preproc_feature_matrix(bool force_preprocessing)
{
	...
	if ( feature_matrix && get_num_preproc() && (!preprocessed || force_preprocessing) )
	{
		...
		for (int i=0; i<get_num_preproc(); i++)
		{
			...
			if (((CStringPreProc*) get_preproc(i))->apply_to_feature_matrix(this) == NULL)
				return false;
		}
		return true;
	}
...
#warning num_features must not correspond with the length of a feature vectore since that one might be preprocessed
	...
	unsigned int fourcc='SFEA'; //id for string features
