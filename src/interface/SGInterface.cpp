...
bool CSGInterface::a_best_path_no_b()
{
	...
	CSGInterfaceMatrix im_p(SGIDT_DREAL);
	get_matrix(im_p);
	DREAL* p=NULL;
	UINT M_p=0;
	UINT N_p=0;
	...
	CSGInterfaceMatrix im_q(SGIDT_DREAL);
	get_matrix(im_q);
	DREAL* q=NULL;
	UINT M_q=0;
	UINT N_q=0;
	...
	CSGInterfaceMatrix im_a(SGIDT_DREAL);
	get_matrix(im_a);
	DREAL* a=NULL;
	UINT M_a=0;
	UINT N_a=0;
	...
	CSGInterfaceVector iv_prob(&prob, 1);
	set_vector(iv_prob);

	CSGInterfaceVector iv_path(path, best_iter+1);
	set_vector(iv_path);
	delete[] path;
...
bool CSGInterface::a_best_path_trans_simple()
{
	...
	CSGInterfaceMatrix im_a_trans(SGIDT_DREAL);
	get_matrix(im_a_trans);
	DREAL* a_trans=NULL;
	UINT M_a_trans=0;
	UINT N_a_trans=0;
	im_a_trans.get(a_trans, M_a_trans, N_a_trans);

	CSGInterfaceMatrix im_seq(SGIDT_DREAL);
	get_matrix(im_seq);
	DREAL* seq=NULL;
	UINT M_seq=0;
	UINT N_seq=0;
	im_seq.get(seq, M_seq, N_seq);
	...
	CSGInterfaceVector iv_prob(prob, nbest);
	set_vector(iv_prob);
	delete[] prob;

	CSGInterfaceMatrix im_path(path, nbest, N_seq);
	set_matrix(im_path);
	delete[] path;
...
bool CSGInterface::a_best_path_no_b_trans()
{
	...
	CSGInterfaceMatrix iv_path(path, nbest, max_best_iter+1);
	set_matrix(iv_path);
	delete[] path;
...
bool CSGInterface::a_get_labels()
{
	...
	CSGInterfaceVector iv(lab, num_labels);
	set_vector(iv);
	delete[] lab;
...
bool CSGInterface::a_obtain_from_position_list()
{
	...
	CSGInterfaceVector iv(SGIDT_INT);
	INT* shifts=NULL;
	INT num_shift=0;
	get_vector(iv);
	iv.get(shifts, num_shift);
		...
		case C_SIMPLE:
			switch (feat->get_feature_type())
			{
				case F_DREAL:
				{
					...
					CSGInterfaceMatrix im(result, num_feat, num_vec);
					set_matrix(im);
					delete[] result;
		...
		case C_STRING:
			switch (feat->get_feature_type())
			{
				case F_CHAR:
				{
					...
					CSGInterfaceStringList isl(list, num_vec);
					set_string_list(isl);
					delete[] list;
				}
				break;
...
bool CSGInterface::a_get_distance_matrix()
{
	...
	CSGInterfaceMatrix im(dmatrix, num_vec1, num_vec2);
	set_matrix(im);
	delete[] dmatrix;
...
bool CSGInterface::a_get_kernel_matrix()
{
	...
	CSGInterfaceMatrix im(kmatrix, num_vec1, num_vec2);
	set_matrix(im);
	delete[] kmatrix;
...
bool CSGInterface::a_set_custom_kernel()
{
	...
	CSGInterfaceMatrix im(SGIDT_DREAL);
	get_matrix(im);
	DREAL* kmatrix=NULL;
	UINT num_feat=0;
	UINT num_vec=0;
	im.get(kmatrix, num_feat, num_vec);
...
bool CSGInterface::a_set_subkernel_weights()
{
	...
	{
		UINT num_subkernels=kernel->get_num_subkernels();
...
bool CSGInterface::a_set_last_subkernel_weights()
{
	...
	if (ktype==K_WEIGHTEDDEGREE)
	{
		...
		if (dim!=(UINT) k->get_degree() || len<1)
			SG_SERROR("Dimension mismatch (should be de(seq_length | 1) x degree)\n");
...
bool CSGInterface::a_get_SPEC_consensus()
{
	...
	CSGInterfaceVector iv(consensus, num_feat);
	set_vector(iv);
	delete[] consensus;
...
bool CSGInterface::a_get_SPEC_scoring()
{
	...
	CSGInterfaceMatrix im(position_weights, num_sym, num_feat);
	set_matrix(im);
...
bool CSGInterface::a_compute_POIM_WD()
{
	...
	UINT num_dfeat=0;
	UINT num_dvec=0;
	im.get(distribution, num_dfeat, num_dvec);
	...
	if (num_dvec!=(UINT) seqlen || num_dfeat!=(UINT) num_sym)
	{
	...
	*/
	...
	im.set(position_weights, num_sym, seqlen);
	set_matrix(im);
	delete[] position_weights;
...
bool CSGInterface::a_get_WD_position_weights()
{
	...
	if (position_weights==NULL)
		iv.set(position_weights, 0);
	else
		iv.set(position_weights, (UINT) len);
	set_vector(iv);
...
bool CSGInterface::a_get_last_subkernel_weights()
{
	...
	if (ktype==K_COMBINED)
	{
		...
		for (INT i=0; i<num_weights; i++)
			weights[i]=w[i];
		CSGInterfaceVector iv(weights, num_weights);
		set_vector(iv);
		delete[] weights;
	...
	CSGInterfaceMatrix im(weights, degree, len);
	set_matrix(im);
...
bool CSGInterface::a_get_kernel_optimization()
{
	...
	switch (kernel->get_kernel_type())
	{
		...
		case K_WEIGHTEDCOMMWORDSTRING:
		{
			...
			CSGInterfaceVector iv(weights, len);
			set_vector(iv);
			delete[] weights;
...
bool CSGInterface::a_plugin_estimate_classify_example()
{
	...
	CSGInterfaceVector iv(&result, 1);
	set_vector(iv);
	return true;
}
...
bool CSGInterface::a_plugin_estimate_classify()
{
	...
	CSGInterfaceVector iv(result, num_vec);
	set_vector(iv);
	delete[] result;
...
bool CSGInterface::a_set_plugin_estimate()
{
	...
	UINT num_probs=0;
	UINT num_vec=0;
	im.get(emission_probs, num_probs, num_vec);
...
bool CSGInterface::a_get_plugin_estimate()
{
	...
	CSGInterfaceMatrix im(result, num_params, 2);
	set_matrix(im);
	delete[] result;
	...
	CSGInterfaceVector iv(model_sizes, 2);
	set_vector(iv);
...
bool CSGInterface::a_get_classifier()
{
	...
	CSGInterfaceMatrix im(bias, brows, bcols);
	set_matrix(im);
	delete[] bias;
	im.set(weights, rows, cols);
	set_matrix(im);
	delete[] weights;
...
bool CSGInterface::a_set_svm()
{
	...
	get_matrix(im);
	DREAL* alphas=NULL;
	UINT num_feat_alphas=0;
	UINT num_vec_alphas=0;
	im.get(alphas, num_feat_alphas, num_vec_alphas);
...
bool CSGInterface::a_get_svm_objective()
{
	...
	CSGInterfaceVector iv(&objective, 1);
	set_vector(iv);
...
bool CSGInterface::a_relative_entropy()
{
	...
	CSGInterfaceVector iv(entropy, pos_N);
	set_vector(iv);
	delete[] entropy;
...
bool CSGInterface::a_entropy()
{
	...
	CSGInterfaceVector iv(entropy, N);
	set_vector(iv);
	delete[] entropy;
...
bool CSGInterface::a_hmm_likelihood()
{
	...
	CSGInterfaceVector iv(&likelihood, 1);
	set_vector(iv);
...
bool CSGInterface::a_get_viterbi_path()
{
	...
	CSGInterfaceVector iv(path, num_feat);
	set_vector(iv);
	delete[] path;
	iv.set(&likelihood, 1);
	set_vector(iv);
...
bool CSGInterface::a_append_hmm()
{
	...
	CSGInterfaceMatrix im_b(SGIDT_DREAL);
	get_matrix(im_b);
	DREAL* b=NULL;
	UINT M_b=0;
	UINT N_b=0;
