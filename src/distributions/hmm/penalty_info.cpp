...
void init_penalty_struct_cache(struct penalty_struct &PEN)
{
	...
	DREAL* cache=new DREAL[PEN.max_len+1] ;
	if (cache)
	{
		DREAL input_value ;
...
struct penalty_struct * read_penalty_struct_from_cell(const mxArray * mx_penalty_info, INT &P)
{
	...
	for (INT i=0; i<P; i++)
	{
		...
		PEN[id].limits = new DREAL[len] ;
		PEN[id].penalties = new DREAL[len] ;
...
DREAL lookup_penalty_svm(const struct penalty_struct *PEN, INT p_value, DREAL *d_values, bool follow_next, 
						DREAL &input_value)
{	
	if (PEN==NULL)
		return 0 ;
	ASSERT(PEN->use_svm>0) ;
	DREAL d_value=d_values[PEN->use_svm-1] ;
    input_value = d_value ;
