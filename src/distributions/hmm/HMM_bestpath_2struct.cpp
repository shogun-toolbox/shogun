...
void CHMM::best_path_2struct(const REAL *seq, INT seq_len, const INT *pos,
							 ...
							 REAL *&PEN_values, REAL *&PEN_input_values, INT &num_PEN_id)
{
			...
			else
			{
				...
				for (INT i=0; i<num_elem; i++)
				{
					...
					for (INT ts=t-1; ts>=0 && pos[t]-pos[ts]<=look_back; ts--)
					{
						...
						REAL input_value ;
						REAL pen_val = lookup_penalty(penalty, pos[t]-pos[ts], svm_value, true, input_value) + segment_sum_value[j] ;
