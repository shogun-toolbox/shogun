...
CHMM::CHMM(CHMM* h, INT number_of_hmm_tables)
{
	...
	if (NUM_PARALLEL<=0)
		CIO::message("NUM_PARALLEL has illegal value") ;
	CIO::message("hmm is using %i separate tables\n",  NUM_PARALLEL) ;
...
CHMM::CHMM(INT N, INT M, CModel* model, REAL PSEUDO, INT number_of_hmm_tables)
{
	NUM_PARALLEL= number_of_hmm_tables ;
	this->N=N;
	this->M=M;
...
CHMM::CHMM(FILE* model_file, REAL PSEUDO, INT number_of_hmm_tables)
{
	NUM_PARALLEL= number_of_hmm_tables ;
...
bool CHMM::initialize(CModel* model, REAL PSEUDO, FILE* modelfile)
{
	//yes optimistic
	bool files_ok=true;
...
//calculates Pr[O_0,O_1, ..., O_t, q_time=S_i| lambda] for 0<= time <= T-1
//Pr[O|lambda] for time > T
REAL CHMM::forward_comp_old(INT time, INT state, INT dimension)
{
	T_ALPHA_BETA_TABLE* alpha_new;
	T_ALPHA_BETA_TABLE* alpha;
	T_ALPHA_BETA_TABLE* dummy;
...
REAL CHMM::backward_comp_old(INT time, INT state, INT dimension)
{
	T_ALPHA_BETA_TABLE* beta_new;
	T_ALPHA_BETA_TABLE* beta;
	T_ALPHA_BETA_TABLE* dummy;
...
REAL CHMM::best_path_no_b(INT max_iter, INT &best_iter, INT *my_path)
{
	delete[] states_per_observation_psi ;
	...
	for (INT t=1; t<max_iter; t++)
	{
		...
		for (INT j=0; j<NN; j++)
		{
			REAL * matrix_a = &transition_matrix_a[j*N] ; // sorry for that
			REAL maxj = delta[0] + matrix_a[0];
			INT argmax=0;
			
			for (INT i=1; i<NN; i++)
			{
				REAL temp = delta[i] + matrix_a[i];
...
//			CIO::message("-- %i %e\n", argmax, maxj) ;
		...
		{ //termination
			REAL maxj=delta[0]+get_q(0);
			INT argmax=0;
			
			for (INT i=1; i<N; i++)
			{
				REAL temp=delta[i]+get_q(i);
...
//			CIO::message("%i %e\n", argmax, maxj) ;
	...
	{ //state sequence backtracking
		...
		{
			my_path[t-1]=get_psi(t, my_path[t], 0);
		}
	}
...
void CHMM::estimate_model_baum_welch(CHMM* train)
{
	...
	if (train->get_p(0)>-0.00001)
	{
		for (i=0; i<N; i++)
		{
			...
			{
				for (j=0; j<M; j++)
					train->set_b(i,j, log(MIN_RAND+((REAL)random()))/REAL(RANDOM_MAX));
			}
		}
	}
...
void CHMM::estimate_model_baum_welch(CHMM* train)
{
	...
	if ((bla) && train->get_q(N-1)>-0.00001)
	{
		...
		{
			if (i<=N-50)
				train->set_p(i, log(MIN_RAND+(random()%RANDOM_MAX)));
			else
				train->set_p(i, -1000);
			...
			if ( i==N-25-1)
				...
				train->set_q(i, log(MIN_RAND+(random()%RANDOM_MAX)));
			CIO::message("q[%d]=%lf\n", i, train->get_q(i));

			if (i>=N-25)
			{
				for (j=0; j<M; j++)
					train->set_b(i,j, log(MIN_RAND+(random()%RANDOM_MAX)));
			}
		}
		train->invalidate_model();
		train->normalize(true);
	}
	//if (train->get_q(N-1)>-0.00001)
	/*	{
		CIO::message("yes2\n");
		for (i=0; i<N; i++)
		{
...
void CHMM::init_model_random()
{
	...
	for (i=0; i<N; i++)
	{
		...
		for (j=0; j<N; j++)
		{
			set_a(i,j, (MIN_RAND+(random()%RANDOM_MAX)));
	...
	for (i=0; i<N; i++)
	{
		set_p(i, (MIN_RAND+(random()%RANDOM_MAX)));
	...
	for (i=0; i<N; i++)
	{
		set_q(i, (MIN_RAND+(random()%RANDOM_MAX)));
	...
	for (i=0; i<N; i++)
	{
		...
		for (j=0; j<M; j++)
		{
			set_b(i,j, (MIN_RAND+(random()%RANDOM_MAX)));
...
void CHMM::init_model_defined()
{
	...
	//initialize a values that have to be learned
	REAL *R=new REAL[N] ;
	for (r=0; r<N; r++) R[r]=(23e-3+((REAL)random()))/REAL(RANDOM_MAX) ;
	i=0; sum=0; k=i; 
	j=model->get_learn_a(i,0);
	while (model->get_learn_a(i,0)!=-1 || k<i)
	{
		...
		else
		{
			...
			k=i;
			sum=0;
			for (INT r=0; r<N; r++) R[r]=(23e-3+((REAL)random()))/REAL(RANDOM_MAX) ;
		}
	}
	delete[] R ; R=NULL ;
	...
	R=new REAL[M] ;
	for (r=0; r<M; r++) R[r]=(23e-3+((REAL)random()))/REAL(RANDOM_MAX) ;
	i=0; sum=0; k=0 ;
	j=model->get_learn_b(i,0);
	while (model->get_learn_b(i,0)!=-1 || k<i)
	{
		...
		else
		{
			...
			k=i;
			sum=0;
			for (INT r=0; r<M; r++) R[r]=(23e-3+((REAL)random()))/REAL(RANDOM_MAX) ;
		}
	}
	delete[] R ; R=NULL ;
	...
	while (model->get_learn_p(i)!=-1)
	{
		set_p(model->get_learn_p(i),(23e-3+((REAL)random()))/((REAL)RANDOM_MAX)) ;
	...
	while (model->get_learn_q(i)!=-1)
	{
		set_q(model->get_learn_q(i),(23e-3+((REAL)random()))/((REAL)RANDOM_MAX)) ;
		...
		if (get_a(i,j)>math.ALMOST_NEG_INFTY)
		  {
		    ...
		    //    num_backward_transitions++ ;
		  } 
	    } ;
	  //CIO::message("%ld  forward trans: %i  backward_trans: %i\n", (long int) this, num_forward_transitions, num_backward_transitions) ;
	} ;
#ifndef NOVIT
...
bool CHMM::load_model(FILE* file)
{
	...
	CIO::message("not normalizing anymore, call normalize_hmm to make sure the hmm is valid!!\n");
	////////!!!!!!!!!!!!!!normalize(); 
	return result;
}
...
bool CHMM::check_model_derivatives_combined()
{
	...
	/*	for (i=0; i<N; i++)
		...
		invalidate_model() ;
		REAL prob_old=exp(model_probability(-1)*p_observations->get_num_vectors()) ;

		set_a(i,j, log(exp(old_a)+delta)) ;
		invalidate_model() ; 
		REAL prob_new=exp(model_probability(-1)*p_observations->get_num_vectors());
		...
		REAL prod_prob=model_probability(-1)*p_observations->get_num_vectors() ;
	...
	i=0;//for (i=0; i<N; i++)
	{
		for (INT j=0; j<M; j++)
		{
			...
			invalidate_model() ;
			REAL prob_old=(model_probability(-1)*p_observations->get_num_vectors()) ;

			set_b(i,j, log(exp(old_b)+delta)) ;
			invalidate_model() ; 
			REAL prob_new=(model_probability(-1)*p_observations->get_num_vectors());
...
bool CHMM::append_model(CHMM* append_model)
{
	...
	CIO::message("cur N:%d M:%d\n", N, M);
	CIO::message("old N:%d M:%d\n", append_model->get_N(), append_model->get_M());
	if (append_model->get_M() == get_M())
	{
		...
		////////!!!!!!!!!!!!!!normalize(); 
		invalidate_model();
		CIO::message("still alive\n");

	}
	else
		CIO::message("number of observations is different for append model, doing nothing!\n");
...
{
#define VAL_MACRO log((default_value == 0) ? ((MIN_RAND+((REAL)random()))/(REAL(RANDOM_MAX/MAX_RAND))) : default_value)
	INT i,j;
...
bool CHMM::permutation_entropy(INT window_width, INT sequence_number)
{
			...
			( sequence_number<0 || sequence_number < p_observations->get_num_vectors()))
	{
		...
		if (sequence_number<0)
		{
			...
			CIO::message("numseq: %d\n", max_sequence);
		}

		CIO::message("min_sequence: %d max_sequence: %d\n", min_sequence, max_sequence);
		for (sequence_number=min_sequence; sequence_number<max_sequence; sequence_number++)
		{
			...
			for (i=0; i<sequence_length-window_width; i++)
			{
				...
				for (j=0; j<get_M(); j++)
				{
					double p=(((REAL)hist[j])+PSEUDO)/(window_width+get_M()*PSEUDO);
