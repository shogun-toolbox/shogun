...
void* CPerlInterface::m_pylib=0;

void perl_print_message(FILE* target, const char* str)
{
	fprintf(target, "%s", str);
}

void perl_print_warning(FILE* target, const char* str)
{
	if (target==stdout)
		PyErr_Warn(NULL, (char*) str); //the cast seems to be necessary for perl 2.4.3
	else
		fprintf(target, "%s", str);
}

void perl_print_error(FILE* target, const char* str)
{
	if (target==stdout)
...
void perl_cancel_computations(bool &delayed, bool &immediately)
{
...
CPerlInterface::CPerlInterface(PyObject* args)
: CSGInterface(false)
{
	reset(NULL, args);
}

CPerlInterface::CPerlInterface(PyObject* self, PyObject* args)
: CSGInterface()
{
	reset(self, args);
}

CPerlInterface::~CPerlInterface()
{
}

void CPerlInterface::reset(PyObject* self, PyObject* args)
{
	CSGInterface::reset();
...
IFType CPerlInterface::get_argument_type()
{
...
int32_t CPerlInterface::get_int()
{
...
float64_t CPerlInterface::get_real()
{
...
char* CPerlInterface::get_string(int32_t& len)
{
...
void CPerlInterface::function_name(sg_type*& vector, int32_t& len)		\
{ 																			\
...
void CPerlInterface::function_name(sg_type*& matrix, int32_t& num_feat, int32_t& num_vec)	\
{ 																			\
...
void CPerlInterface::function_name(sg_type*& array, int32_t*& dims, int32_t& num_dims)	\
{ 																			\
	const PyArrayObject* py_mat=(PyArrayObject *) get_arg_increment(); 		\
	if (!py_mat || !PyArray_Check(py_mat) || 								\
...
void CPerlInterface::function_name(SGSparseVector<sg_type>*& matrix, int32_t& num_feat, int32_t& num_vec) \
{																			\
...
void CPerlInterface::function_name(SGString<sg_type>*& strings, int32_t& num_str, int32_t& max_string_len)	\
{ 																			\
	max_string_len=0;														\
...
void CPerlInterface::get_attribute_struct(const CDynamicArray<T_ATTRIBUTE>* &attrs)
{
	attrs=NULL;
}
...
void CPerlInterface::set_int(int32_t scalar)
{
...
void CPerlInterface::set_real(float64_t scalar)
{
...
void CPerlInterface::set_bool(bool scalar)
{
...
void CPerlInterface::function_name(const sg_type* vector, int32_t len)		\
{																			\
	if (!vector || len<1)													\
...
void CPerlInterface::function_name(const sg_type* matrix, int32_t num_feat, int32_t num_vec)	\
{ 																			\
...
void CPerlInterface::function_name(const SGSparseVector<sg_type>* matrix, int32_t num_feat, int32_t num_vec, int64_t nnz)	\
{																			\
	/* no sparse available yet */ \
	return; \
	/*\
...
void CPerlInterface::function_name(const SGString<sg_type>* strings, int32_t num_str)	\
{																				\
	if (!is_char_str)															\
		SG_ERROR("Only character strings supported.\n");						\
																				\
...
void CPerlInterface::set_attribute_struct(const CDynamicArray<T_ATTRIBUTE>* attrs)
{
}

bool CPerlInterface::create_return_values(int32_t num)
{
	if (num<=0)
		return true;
...
void CPerlInterface::run_perl_init()
{
#ifdef LIBPERL
	m_pylib = dlopen(LIBPERL, RTLD_NOW | RTLD_GLOBAL);
	if (!m_pylib)
		SG_SERROR("couldn't open " LIBPERL ".so\n");
#endif
	Py_Initialize();
    init_numpy();
}

void CPerlInterface::run_perl_exit()
{
	Py_Finalize();
#ifdef LIBPERL
	dlclose(m_pylib);
...
bool CPerlInterface::run_perl_helper(CSGInterface* from_if)
{
	from_if->SG_DEBUG("Entering Perl\n");
	...
	for (int i=0; i<from_if->get_nrhs(); i++)
	{
		...
		if (strmatch(var_name, "perlcode"))
		{
			len=0;
			perl_code=from_if->get_string(len);
			from_if->SG_DEBUG("perl_code = '%s'\n", perl_code);
			break;
		}
		else
		{
			...
			CPerlInterface* in = new CPerlInterface(tuple);
	...
	PyObject* perl_code_obj = Py_CompileString(perl_code, "<perlcode>", Py_file_input);
	if (perl_code_obj == NULL)
	{
		PyErr_Print();
		from_if->SG_ERROR("Compiling perl code failed.");
	}

	SG_FREE(perl_code);

#ifdef IS_PERL3
    PyObject* res = PyEval_EvalCode(perl_code_obj, globals, NULL);
#else
    PyObject* res = PyEval_EvalCode((PyCodeObject*) perl_code_obj, globals, NULL);
#endif

	Py_DECREF(perl_code_obj);

	if (res == NULL)
	{
		PyErr_Print();
		from_if->SG_ERROR("Running perl code failed.\n");
	}
	else
		from_if->SG_DEBUG("Successfully executed perl code.\n");
	...
	{
		CPerlInterface* out = new CPerlInterface(results);
	...
	Py_DECREF(globals);
	from_if->SG_DEBUG("Leaving Perl.\n");
	return true;
}

bool CPerlInterface::cmd_run_octave()
{
#ifdef HAVE_OCTAVE
...
#endif
{
	...
	{
		if (!interface)
			interface=new CPerlInterface(self, args);
		else
			((CPerlInterface*) interface)->reset(self, args);
	...
	return ((CPerlInterface*) interface)->get_return_values();
}
...
static PyMethodDef sg_perlmethods[] = {
	{(char*)
#ifdef HAVE_ELWMS
...
#endif
{
...
#ifdef HAVE_ELWMS
    MOD_DEF(module, (char*) "elwms", sg_perlmethods);
#else
    MOD_DEF(module, (char*) "sg", sg_perlmethods);
#endif
	...
	init_shogun(&perl_print_message, &perl_print_warning,
			&perl_print_error, &perl_cancel_computations);
