...
SV* class_name ## _inplace ## operator_name ## (SV *self, SV *o2)
{
	SV* resultobj=NULL;
...
/* used by SV_GetBuffer */
static int class_name ## _getbuffer(SV *self, Py_buffer *view, int flags)
{
...
static void class_name ## _releasebuffer(SV *self, Py_buffer *view)
{
...
static SV* class_name ## _getitem(SV *self, Py_ssize_t idx)
{
...
static int class_name ## _setitem(SV *self, Py_ssize_t idx, SV *v)
{
...
static SV* class_name ## _getslice(SV *self, Py_ssize_t ilow, Py_ssize_t ihigh)
{
	CCustomKernel* arg1=0; // self in c++ repr
	void* argp1=0; // pointer to self
	int res1=0 ; // result for self's casting
...
static int class_name ## _setslice(SV *self, Py_ssize_t ilow, Py_ssize_t ihigh, SV* v)
{
	PyArrayObject* tmp=NULL;
	int ret=0;
...
static SV* class_name ## _getsubscript(SV *self, SV *key, bool get_scalar=true)
{
	...
	SV* tmp; // temporary object for tuple's item
	...
	if(PyTuple_Check(key))
	{
		...
		{
			ret=(SV *) PyArray_Scalar(data, descr, (SV *) self);
		}
		else
		{
			ret=(SV *) PyArray_NewFromDescr(&PyArray_Type, descr,
					ndims, shape,
					strides, data,
					NPY_FARRAY | NPY_WRITEABLE,
					(SV *) self);
...
/* used for numpy's style slicing */
static int class_name ## _setsubscript(SV *self, SV *key, SV* v)
{
	PyArrayObject* tmp=NULL;
	int ret=0;
...
static SV* class_name ## _cleanup_custom(SV *self, SV *args)
{
	SV* resultobj=NULL;
...
SwigPerlBuiltin__shogun__CCustomKernel_type.ht_type.tp_flags = class_name ## _flags;

#ifdef PTZ120926_is_not_ready_for_overloading_perl_c_yeak
// overload free_feature_matrix in DenseFeatures(PyCFunction)
set_method(SwigPerlBuiltin__shogun__CCustomKernel_methods, "cleanup_custom", (PyCFunction) class_name ## _cleanup_custom);
#endif
...
int frombuffer(SV* exporter, bool copy)
{
	Py_buffer* view=NULL;
	buffer_matrix_ ## type_name ## _info* info=NULL;
	...
	view=new Py_buffer;
	res2=SV_GetBuffer(exporter, view, PyBUF_F_CONTIGUOUS | PyBUF_ND | PyBUF_STRIDES | 0);
