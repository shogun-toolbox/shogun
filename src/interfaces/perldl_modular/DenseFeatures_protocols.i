...
static SV* class_name ## _getitem(SV *self, STRLEN idx)
{
...
static int class_name ## _setitem(SV *self, STRLEN idx, SV *v)
{
...
static SV* class_name ## _getslice(SV *self, STRLEN ilow, STRLEN ihigh)
{
	CDenseFeatures< type_name >* arg1=0; // self in c++ repr
	void* argp1=0; // pointer to self
	int res1=0 ; // result for self's casting
...
static int class_name ## _setslice(SV *self, STRLEN ilow, STRLEN ihigh, SV* v)
{
	PyArrayObject* tmp=NULL;
	int ret=0;
...
static SV* class_name ## _getsubscript(SV *self, SV *key, bool get_scalar=true)
{
	...
	STRLEN feat_step=0;
	STRLEN vec_step=0;
	STRLEN feat_slicelength=0;
	STRLEN vec_slicelength=0;
...
static SV* class_name ## _free_feature_matrix(SV *self, SV *args)
{
	SV* resultobj=0;
