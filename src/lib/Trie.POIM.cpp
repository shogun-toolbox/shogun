...
void CTrie::POIMs_init()
{
  const INT N = length;
  INT i;
  for( i = 0; i < N; ++i ) {
    POIMs_init_helper( trees[i], 0 );
  }
}

void CTrie::POIMs_init_helper( const Trie& nodeIdx, const int depth )
{
  const Trie* node = &TreeMem[ node ];
  {
  ...
  if( depth < degree-1 ) {
        ...
        POIMs_init_helper( childIdx, depth+1 );
			}
		}
  } else {
...
void CTrie::POIMs_calc_SLR()
{
  ...
  for( i = 0; i < N-1; ++i ) {
    for( symLeft = NUM_SYMS; symLeft < depth*NUM_SYMS; ++symLeft ) {
      leftSubtreeIdcs[ symLeft ] = NO_CHILD;  // AID DEBUGGIN
    }
    POIMs_calc_SLR_helper( trees[i], leftSubtreeIdcs, 0 );
...
void CTrie::POIMs_calc_SLR_helper( const INT nodeIdx, INT* const leftSubtreeIdcs, const int depth,
                                   DREAL* S, DREAL* L, DREAL* R )
{
  ...
  if( depth < degree-1 ) {
		...
		for( symRight = 0; symRight < NUM_SYMS; ++symRight ) {
			...
			if( childIdx != NO_CHILD ) {
        POIMs_calc_SLR_helper( childIdx, leftSubtreeIdcs+NUM_SYMS, depth+1 );
      }
		}
    *S = node->S;
    *L = node->L;
