...
CMultilabelLabels::CMultilabelLabels(mclass_t num_classes)
: CLabels()
{
	m_num_labels  = 0;
...
CMultilabelLabels::CMultilabelLabels(int32_t num_labels, mclass_t num_classes)
: CLabels()
{
	ASSERT(num_labels >= 0);
...
int32_t CMultilabelLabels::get_num_labels() const
{
	return m_num_labels;
}

mclass_t CMultilabelLabels::get_num_classes() const
{
	return m_num_classes;
}

void CMultilabelLabels::set_labels(SGVector<mclass_t> * labels)
{
...
SGVector<int32_t> **
CMultilabelLabels::get_class_labels() const
{
	SGVector<int32_t> ** labels_list = SG_MALLOC(SGVector<int32_t> *, get_num_classes());

	int32_t * num_label_idx = SG_MALLOC(int32_t, get_num_classes());
	...
	for (int32_t label_j=0; label_j < get_num_labels(); label_j++) {
		for (int32_t c_pos=0; c_pos < m_labels[label_j].vlen; c_pos++) {
			mclass_t class_i = m_labels[label_j][c_pos];
	...
	for (mclass_t class_i=0; class_i < get_num_classes(); class_i++)
	{
		labels_list[class_i] = new SGVector<int32_t>(num_label_idx[class_i]);
	}
...
SGVector<D> CMultilabelLabels::to_dense(SGVector<S> * sparse, int32_t dense_len, D d_true, D d_false)
{
...
template SGVector<float64_t> CMultilabelLabels::to_dense<mclass_t, float64_t>(SGVector<mclass_t> *, int32_t, float64_t, float64_t);
template SGVector<int32_t> CMultilabelLabels::to_dense<mclass_t, int32_t>(SGVector<mclass_t> *, int32_t, int32_t, int32_t);
template SGVector<float64_t> CMultilabelLabels::to_dense<int32_t, float64_t>(SGVector<int32_t> *, int32_t, float64_t, float64_t);

void CMultilabelLabels::set_label(int32_t j, SGVector<mclass_t> label)
{
	ASSERT(j < get_num_labels());
	m_labels[j] = label;
}

void CMultilabelLabels::set_class_labels(SGVector <int32_t> ** labels_list)
{
	...
	for (mclass_t class_i=0; class_i < get_num_classes(); class_i++) {
		for (int32_t l_pos=0; l_pos < labels_list[class_i]->vlen; l_pos++) {
			...
			REQUIRE(label_j < get_num_labels(),
			    ...
			    class_i, get_num_classes(), label_j, get_num_labels(), l_pos);
...
void CMultilabelLabels::display() const
{
	...
	for (int32_t class_i = 0; class_i < get_num_classes(); class_i++) {
		printf("  yC_{class_i=%d}", class_i);
	        ...
	        SGVector<float64_t> dense = to_dense<int32_t, float64_t>(labels_list[class_i], get_num_labels(), +1, -1);
	...
	for (int32_t j = 0; j < get_num_labels(); j++) {
		printf("  y_{j=%d}", j);
	        ...
	        SGVector<float64_t> dense = to_dense<mclass_t, float64_t>(&m_labels[j], get_num_classes(), +1, -1);
		dense.display_vector("");
	}
	return;
}

void CMultilabelLabels::save(const char *fname)
{
...
void CMultilabelLabels::load_info(const char *fname, int32_t & num_labels, mclass_t & num_classes)
{
...
CMultilabelLabels * CMultilabelLabels::load(const char *fname)
{
	...
	SGVector<mclass_t> * output_rows = SG_CALLOC(SGVector<mclass_t>, num_labels);
	...
	while (std::getline(labelfile, line)) {
		...
		output_rows[label_j] = SGVector<mclass_t>(SGVector<mclass_t>::clone_vector(temp, num_label_classes), num_label_classes);
		label_j++;
	}

	ASSERT(label_j == num_labels);

	CMultilabelLabels *outputs = new CMultilabelLabels(num_labels, num_classes);
