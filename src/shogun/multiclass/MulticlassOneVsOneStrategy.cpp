...
void CMulticlassOneVsOneStrategy::register_parameters()
{
	...
	SG_WARNING("{}::CMulticlassOneVsOneStrategy(): register parameters!\n", get_name());
}
...
void CMulticlassOneVsOneStrategy::rescale_outputs(SGVector<float64_t> outputs)
{
	...
	if(tot!=m_num_machines)
		SG_ERROR("{}::rescale_output(): size(outputs) is not num_machines.\n", get_name());
	...
	switch(get_prob_heuris_type())
	{
		...
		case PROB_HEURIS_NONE:
			break;
		default:
			SG_ERROR("{}::rescale_outputs(): Unknown OVO probability heuristic type!\n", get_name());
			break;
	}
}
		...
		const SGVector<int32_t> indx1, const SGVector<int32_t> indx2)
{
	...
	{
		SG_ERROR("{}::rescale_heuris_price(): size(outputs) = {} != m_num_machines = {}\n",
		...
		const SGVector<int32_t> indx1, const SGVector<int32_t> indx2)
{
	if (m_num_machines != outputs.vlen)
	{
		SG_ERROR("{}::rescale_heuris_hastie(): size(outputs) = {} != m_num_machines = {}\n",
				get_name(), outputs.vlen, m_num_machines);
	}
		...
		const SGVector<int32_t> indx1, const SGVector<int32_t> indx2)
{
	if (m_num_machines != outputs.vlen)
	{
		SG_ERROR("{}::rescale_heuris_hamamura(): size(outputs) = {} != m_num_machines = {}\n",
				get_name(), outputs.vlen, m_num_machines);
	}
