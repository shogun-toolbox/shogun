...
bool CMulticlassSVM::init_machines_for_apply(CFeatures* data)
{
	...
	if (!lhs && m_kernel->get_kernel_type()!=K_COMBINED)
		error("{}: No left hand side specified\n", get_name());
...
bool CMulticlassSVM::load(FILE* modelfl)
{
	...
	if (fscanf(modelfl,"%15s\n", char_buffer)==EOF)
		error("error in svm file, line nr:{}\n", line_number);
	else
	{
		char_buffer[15]='\0';
		if (strcmp("%MultiClassSVM", char_buffer)!=0)
			error("error in multiclass svm file, line nr:{}\n", line_number);
	...
	if (m_machines->get_num_elements() != int_buffer)
		error("Mismatch in number of svms: m_num_svms={} vs m_num_svms(file)={}\n", m_machines->get_num_elements(), int_buffer);
	...
	for (int32_t n=0; n<m_machines->get_num_elements(); n++)
	{
		...
		else
		{
			...
			if (svm_idx != n)
				error("svm index mismatch n={}, n(file)={}\n", n, svm_idx);
		...
		io::info("loading {} support vectors for svm {}\n",int_buffer, svm_idx);
