...
void CKNN::train_kd(SGMatrix<float64_t> features, SGVector<float64_t> labels)
{
	assert (features.num_cols == labels.vlen);
...
vector<vector<float> > CKNN::featureToVector(SGMatrix<float64_t> features)
{
...
KDTNode* CKNN::construct_helper(KDTNode*& nodep, int dimension, int startIndex,
...
void CKNN::sortByDimension(vector<vector<float> >& elements, int start, int end, int currentDim)
{
...
int CKNN::sortPartition(vector<vector<float> >& elements, int start, int end, int currentDim)
{
...
vector<float> CKNN::FindNearestNeighbor(vector<float> location)
{
...
vector<float> CKNN::FindNearestNeighborHelper(vector<float> location, KDTNode*& nodep, vector<float> bestValue, double bestDistance, int depth)
{
	if (nodep == NULL)
		return bestValue;
...
double CKNN::distanceByEuclidean(vector<float> v1, vector<float> v2)
{
	double distance = 0.0;
	double sum = 0.0;
...
vector<vector<float> > CKNN::FindNearestNNeighbor(vector<float> location, int numNeighbors)
{
	assert(!location.empty());
...
void CKNN::FindNearestNNeighborHelper(vector<float> location, KDTNode*& nodep, vector<float>& bestValue, double& bestDistance,
...
void CKNN::sortByDistance(vector<vector<float> >& valuesList, int start, int end, vector<float> location)
{
	// top = subscript of beginning of array
	// bottom = subscript of end of array
...
int CKNN::sortDistancePartition(vector<vector<float> >& valuesList, int start, int end, vector<float> location)
{
...
float CKNN::apply_one_kd(vector<float> location, int k_nn)
{
