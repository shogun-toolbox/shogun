...
CKNNSolver::CKNNSolver(const index_t k, const float64_t q, const index_t num_classes, const index_t min_label, const SGVector<index_t> train_labels)
: CDistanceMachine()
{
	init();
...
index_t CKNNSolver::choose_class(float64_t* classes, const index_t* train_lab) const
{
...
void CKNNSolver::choose_class_for_multiple_k(index_t* output, index_t* classes, const index_t* train_lab, const index_t step) const
{
	//compute histogram of class outputs of the first k nearest neighbours
	memset(classes, 0, sizeof(index_t)*m_num_classes);
