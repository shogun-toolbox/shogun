...
void CMulticlassOneVsRestStrategy::rescale_outputs(SGVector<float64_t> outputs)
{
	switch(get_prob_heuris_type())
	{
		case OVA_NORM:
			...
			break;
		case OVA_SOFTMAX:
			SG_ERROR("Need to specify sigmoid parameters!\n");
			break;
		case PROB_HEURIS_NONE:
			break;
		default:
			SG_ERROR("Unknown OVA probability heuristic type!\n");
			break;
	}
}
...
void CMulticlassOneVsRestStrategy::rescale_heuris_norm(SGVector<float64_t> outputs)
{
	...
	{
		SG_ERROR("size(outputs) = %d != m_num_classes = %d\n",
				outputs.vlen, m_num_classes);
	}
