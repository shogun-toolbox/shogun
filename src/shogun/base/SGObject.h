...
#define SG_ADD(...) VARARG(SG_ADD, __VA_ARGS__)
	...
	public:
		...
		typedef rxcpp::subjects::subject<std::shared_ptr<ObservedValue>>
		    SGSubject;
		...
		typedef rxcpp::subscriber<
		    std::shared_ptr<ObservedValue>,
		    rxcpp::observer<
		        std::shared_ptr<ObservedValue>, void, void, void, void>>
		    SGSubscriber;
		...
		template <
		    typename T,
		    typename std::enable_if_t<!is_sg_base<T>::value>* = nullptr>
		...
		template <
		    ...
		    class X = typename std::enable_if<is_sg_base<T>::value>::type>
		std::shared_ptr<T>
		get(const std::string& name, index_t index, std::nothrow_t) const
		{
			...
			if (sgo_details::dispatch_array_type<T>(
			        shared_from_this(), name, get_lambda))
			{
				ASSERT(result);
		...
		get(const std::string& name, std::nothrow_t) const noexcept;
		 ...
		 */
		std::shared_ptr<SGObject>
		get(const std::string& name, index_t index) const;
		...
		template <
		    ...
		    typename std::enable_if_t<
		        !is_string<T>::value && !is_sg_base<T>::value>* = nullptr>
		...
		void run(const std::string& name) const noexcept(false)
		{
			...
			{
				SG_ERROR(
				    "Failed to run function %s::%s", get_name(), name.c_str())
			}
		}
		 ...
		 */
		template <class T>
		static T* as(std::shared_ptr<SGObject> sgo)
		{
	...
	protected:
...
#ifndef SWIG
		/** Puts a pointer to a (lazily evaluated) const function into the
		 * parameter map.
		 *
		 * @param name name of the parameter
		 ...
		 * map. The bound function can modify the class members and can only be
	...
	protected:
		 ...
		 * Shogun can only instantiate empty class instances from its source
		 * tree.
		 *
	...
	protected:
		 ...
		 * @param properties AnyParameterProperties used to register the
		 * observed value.
		 */
		template <class T>
		void observe(
		    ...
		    const AnyParameterProperties properties) const
		{
			...
			auto obs = std::make_shared<ObservedValueTemplated<T>>(
			    step, name, static_cast<T>(clone_utils::clone(value)),
			    properties);
			this->observe(obs);
		}
 ...
 */
	...
	namespace sgo_details
	{
		...
		template <typename T>
		std::shared_ptr<SGObject> get_if_possible(
		    const std::shared_ptr<const SGObject>& obj, const std::string& name,
		    GetByName)
		{
		...
		template <typename T>
		std::shared_ptr<SGObject> get_dispatch_all_base_types(
		    const std::shared_ptr<const SGObject>& obj, const std::string& name,
		    T&& how)
		{
		...
		template <class T>
		std::shared_ptr<SGObject> get_by_tag(
		    const std::shared_ptr<const SGObject>& obj, const std::string& name,
		    T&& how)
		{
