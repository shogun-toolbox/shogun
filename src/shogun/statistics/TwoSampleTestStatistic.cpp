...
SGVector<float64_t> CTwoSampleTestStatistic::sample_null()
{
	SG_DEBUG("entering CTwoSampleTestStatistic::sample_null()\n")

	REQUIRE(m_p_and_q, "CTwoSampleTestStatistic::sample_null(): "
	...
	SG_DEBUG("leaving CTwoSampleTestStatistic::sample_null()\n")
	return results;
}

float64_t CTwoSampleTestStatistic::compute_p_value(
		float64_t statistic)
{
	float64_t result=0;
	...
	{
		SG_ERROR("CTwoSampleTestStatistics::compute_p_value(): Unknown"
...
float64_t CTwoSampleTestStatistic::compute_threshold(
		float64_t alpha)
{
	float64_t result=0;
	...
	{
		SG_ERROR("CTwoSampleTestStatistics::compute_threshold():"
...
void CTwoSampleTestStatistic::set_p_and_q(CFeatures* p_and_q)
{
...
CFeatures* CTwoSampleTestStatistic::get_p_and_q()
{
	SG_REF(m_p_and_q);
	return m_p_and_q;
}
