...
struct LEARN_PARM {
  ...
  index_t svm_newvarsinqp;
  /** size of kernel cache in megabytes */
  index_t kernel_cache_size;
...
class CSVMLight : public CSVM
{
 public:
  ...
  int32_t optimize_to_convergence(
	  int32_t* docs, index_t* label, int32_t totdoc, SHRINK_STATE* shrink_state,
	  index_t* inconsistent, float64_t* a, float64_t* lin, float64_t* c,
	  TIMING* timing_profile, float64_t* maxdiff, index_t heldout,
	  index_t retrain);
   ...
   */
  void add_to_index(index_t* index, index_t elem);
   ...
   * @return something inty
   */
  index_t compute_index(index_t* binfeature, index_t range, index_t* index);
  ...
  void optimize_svm(
	  ...
	  float64_t eq_target, index_t* chosen, index_t* active2dnum,
	  int32_t totdoc, index_t* working2dnum, index_t varnum, float64_t* a,
	  float64_t* lin, float64_t* c, float64_t* aicache, QP* qp,
  ...
  void compute_matrices_for_optimization(
	  ...
	  float64_t* a, float64_t* lin, float64_t* c, index_t varnum,
	  int32_t totdoc, float64_t* aicache, QP* qp);
  ...
  int32_t calculate_svm_model(
	  ...
	  float64_t* a_old, float64_t* c, index_t* working2dnum,
	  index_t* active2dnum);
  ...
  int32_t check_optimality(
	  index_t* label, float64_t* a, float64_t* lin, float64_t* c,
	  int32_t totdoc, float64_t* maxdiff, float64_t epsilon_crit_org,
	  index_t* misclassified, index_t* inconsistent, index_t* active2dnum,
	  index_t* last_suboptimal_at, index_t iteration);
  ...
  void call_mkl_callback(float64_t* a, index_t* label, float64_t* lin);
  ...
  int32_t select_next_qp_subproblem_grad(
	  index_t* label, float64_t* a, float64_t* lin, float64_t* c,
	  int32_t totdoc, index_t qp_size, index_t* inconsistent,
	  index_t* active2dnum, index_t* working2dnum, float64_t* selcrit,
	  index_t* select, index_t cache_only, index_t* key, index_t* chosen);
  ...
  int32_t select_next_qp_subproblem_rand(
	  ...
	  int32_t totdoc, index_t qp_size, index_t* inconsistent,
	  index_t* active2dnum, index_t* working2dnum, float64_t* selcrit,
	  index_t* select, index_t* key, index_t* chosen, index_t iteration);
   ...
   */
  void
  select_top_n(float64_t* selcrit, index_t range, index_t* select, index_t n);
