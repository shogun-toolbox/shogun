...
void MKLMulticlassGradient::linesearch(std::vector<float64_t> & finalbeta,const std::vector<float64_t> & oldweights)
{
	...
	// curgamma from init: arcsin on highest dim ^p/2 !!! and divided everthing by its cos
		...
		for(int32_t i=numkernels-1; i>= 1 ;--i)
		{
			...
			if(i<numkernels-1)
			{
				if( cos(curgamma[i])<=0)
				{
               SG_INFO("linesearch(...): at i {} cos(curgamma[i-1])<=0 {}\n",i, cos(curgamma[i-1]))
					//curgamma[i-1]=pi4/2;
				}
			}
			...
			for(int32_t k=numkernels-2; k>= 1 ;--k) // k==0 not necessary
			{
				if(cos(curgamma[i-1])>0)
				{
					...
					if(tmpbeta[k]>1)
					{
                  SG_INFO("linesearch(...): at k {} tmpbeta[k]>1 {}\n",k, tmpbeta[k])
					}
	...
	while(!finished)
	{
		...
		SG_INFO("linesearch(...): objectives at i {}\n",minval)
...
void MKLMulticlassGradient::computeweights(std::vector<float64_t> & weights2)
{
	...
	// for(size_t i=0;i<weights2.size();++i)
	// {
	//    SG_INFO("MKLMulticlassGradient::computeweights(...): oldweights {}\n",initw[i])
	//	}
	SG_INFO("MKLMulticlassGradient::computeweights(...): newweights at iter {} normdiff {}\n",numiter,norm)
	for(size_t i=0;i<weights2.size();++i)
	{
	SG_INFO(" {}",weights2[i])
	}
