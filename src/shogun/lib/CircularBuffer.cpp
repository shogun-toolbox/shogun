...
int32_t CircularBuffer::push(SGVector<char> source)
{
	...
	{
		SG_ERROR("CircularBuffer::push(SGVector<char>):: Invalid parameters! Source shouldn't be NULL or zero sized\n");
		return -1;
	}
...
int32_t CircularBuffer::push(FILE* source, int32_t source_size)
{
	...
	{
		SG_ERROR("CircularBuffer::push(FILE*, int32_t):: Invalid parameters! Source shouldn't be NULL or zero sized\n");
		return -1;
	}
...
bool CircularBuffer::has_next()
{
	if (m_tokenizer==NULL)
	{
		SG_ERROR("CircularBuffer::has_next():: Tokenizer is not initialized\n");
		return false;
	}
...
index_t CircularBuffer::next_token_idx(index_t &start)
{
	...
	if (m_tokenizer==NULL)
	{
		SG_ERROR("CircularBuffer::next_token_idx(index_t&):: Tokenizer is not initialized\n");
		return 0;
	}
					...
					bool from_buffer_begin)
{
	if (source==NULL || source_size==0)
	{
		SG_ERROR("CircularBuffer::append_chunk(const char*, int32_t, bool):: Invalid parameters!\
					...
					bool from_buffer_begin)
{
	...
	{
		SG_ERROR("CircularBuffer::detach_chunk(...):: Invalid parameters! Pointers are NULL\n");
		return;
	}
