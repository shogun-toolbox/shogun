 ...
 *
 * Authors: Soeren Sonnenburg, Sergey Lisitsyn, Yuyu Zhang, Weijie Lin, 
 *          Evan Shelhamer, Bjoern Esser, Saurabh Goyal
 */
...
bool CTrie<Trie>::compare(const CTrie<Trie> & other)
{
	...
	for (int32_t i=0; i<length; i++)
		if (!compare_traverse(trees[i], other, other.trees[i]))
			...
			SG_DEBUG("two tries at {} identical\n", i)
