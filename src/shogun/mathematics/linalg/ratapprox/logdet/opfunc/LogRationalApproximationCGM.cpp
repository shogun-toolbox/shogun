...
CLogRationalApproximationCGM::CLogRationalApproximationCGM(
	...
	: CRationalApproximation(linear_operator,
...
float64_t CLogRationalApproximationCGM::solve(
	SGVector<float64_t> sample)
{
	...
	REQUIRE(m_linear_operator->get_dimension()==sample.vlen,
		"Dimension mismatch! %d vs %d\n", m_linear_operator->get_dimension(), sample.vlen);
	REQUIRE(m_negated_shifts.vlen==m_weights.vlen,
