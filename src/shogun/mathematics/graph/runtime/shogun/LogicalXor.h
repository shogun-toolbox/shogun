...
#ifndef SHOGUN_DETAIL_LOGICAL_XOR_SHOGUN_H_
#define SHOGUN_DETAIL_LOGICAL_XOR_SHOGUN_H_

#include <shogun/mathematics/graph/nodes/LogicalXor.h>
#include <shogun/mathematics/graph/ops/shogun/LogicalXor.h>
			...
			namespace shogun
			{
				IGNORE_IN_CLASSLIST class LogicalXorShogun
				    : public ShogunBinaryRuntimeNode<
				          LogicalXorShogun, node::LogicalXor, OutputNode>
				{
				public:
					LogicalXorShogun() : ShogunBinaryRuntimeNode()
					{
					}
					...
					build_implementation_(
						...
						return std::make_shared<OutputNode>(
						    std::make_shared<op::LogicalXorShogun>(graph_node),
						    node1, node2);
					}
				};
			} // namespace shogun
