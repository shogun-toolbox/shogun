	...
	namespace graph
	{
		namespace detail
		{
			namespace ngraph
			{
				...
				::ngraph::element::Type_t get_ngraph_type_from_enum(element_type type)
				{
					switch (type)
					{
				...
				::ngraph::PartialShape to_ngraph_partial_shape(const Shape& shape)
				{
				...
				public:
					...
					[[nodiscard]] std::shared_ptr<::ngraph::Node> build_input(const std::shared_ptr<node::Node>& node) const
					{
					...
					[[nodiscard]] std::shared_ptr<::ngraph::Node> build_implementation(const std::shared_ptr<node::Node>& node) const final
					{
