 ...
 *
 * Authors: Gil Hoben (modified from https://docs.microsoft.com/en-us/cpp/intrinsics/cpuid-cpuidex?view=vs-2019)
 */
...
CPUArch::CPUArch(): instructions(SIMD::NONE)
{
    ...
    if (InstructionSet::AVX512F)
        enable_simd(instructions, SIMD::AVX512F);
    #else
    io::warn("There is no support for runtime CPU architecture detection with the provided compiler. Defaulting to SSE2 instructions.")
    enable_simd(instructions, SIMD::SSE2);
    #endif
}
