		...
		float64_t alpha, float64_t& conf_int_low, float64_t& conf_int_up)
{
	...
	float64_t t=CMath::abs(boost::math::detail::inverse_students_t((float64_t)deg, alpha, 1- alpha, boost::math::policies::policy<>()));
...
{
	return 0 - 1 * sqrt(2) * boost::math::erfc_inv(2 * y0);
}
