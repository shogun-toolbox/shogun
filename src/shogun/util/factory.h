...
#define BASE_CLASS_FACTORY(T, factory_name)                                    \
	...
	std::shared_ptr<Features> features(std::shared_ptr<File> file, machine_int_t primitive_type = PT_FLOAT64)
	{
	...
	std::shared_ptr<Features> string_features(
	    std::shared_ptr<File> file, machine_int_t alphabet_type = DNA,
	    machine_int_t primitive_type = PT_CHAR)
	{
		...
		{
		case PT_CHAR:
			return std::make_shared<StringFeatures<char>>(file, static_cast<EAlphabet>(alphabet_type));
			break;
		default:
			SG_SNOTIMPLEMENTED
		}
	...
	template <typename LT,
	          std::enable_if_t<
	              std::is_base_of<DenseLabels, typename std::remove_pointer<
	                                                LT>::type>::value,
	              LT>* = nullptr>
