	...
	const TSGDataType* type, index_t* len_read_y, index_t* len_read_x)
{
	...
	case CT_VECTOR: case CT_SGVECTOR:
		if (fscanf(m_file->m_fstream, "%" SCNi64  " ", len_read_y) != 1)
			return false;
		*len_read_x = 1;
		break;
	case CT_MATRIX: case CT_SGMATRIX:
		if (fscanf(
		        m_file->m_fstream, "%" SCNi64  " %" SCNi64  " ", len_read_y,
		        len_read_x) != 2)
			return false;
		break;
	case CT_UNDEFINED:
...
{
	if (fscanf(m_file->m_fstream, "%" PRIi64, length) != 1)
		return false;
...
{
	if (fscanf(m_file->m_fstream, "%" PRIi64, feat_index) != 1)
		return false;
	if (fgetc(m_file->m_fstream) != ' ') return false;
	if (fgetc(m_file->m_fstream) != CHAR_ITEM_BEGIN) return false;
