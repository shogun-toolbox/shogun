...
public:
	...
	{
		SG_DEBUG("writing floatmax_t with value %Lf\n", *v);
		uint64_t msb, lsb;
	...
	{
		SG_DEBUG("writing complex128_t with value (%f, %f)\n", v->real(), v->imag());
		m_json_writer.StartArray();
	...
	{
		SG_DEBUG("writing std::string with value %s\n", v->c_str());
	...
	{
		SG_DEBUG("writing matrix of size: %d x %d\n", *rows, *cols);
		m_json_writer.StartArray();
		if (*cols == 0 || *rows == 0)
		{
			m_json_writer.EndArray();
	...
	{
		SG_DEBUG("writing vector of size: %d\n", *size);
		m_json_writer.StartArray();
		if (*size == 0)
			m_json_writer.EndArray();
		else
	...
	{
		SG_DEBUG("writing std::vector of size: %d\n", *size);
		m_json_writer.StartArray();
		if (*size == 0)
			m_json_writer.EndArray();
		else
	...
	{
		SG_DEBUG("writing map of size: %d\n", *size);
		m_json_writer.StartArray();
		if (*size == 0)
		{
			m_json_writer.EndArray();
