...
public:
	...
	{
		SG_DEBUG("writing bool with value {}\n", *v);
		m_json_writer.Bool(*v);
		close_container();
	}
	void on(char* v) override
	{
		SG_DEBUG("writing char with value {}\n", *v);
		m_json_writer.Int(*v);
		close_container();
	}
	void on(int8_t* v) override
	{
		SG_DEBUG("writing int8_t with value {}\n", *v);
		m_json_writer.Int(*v);
		close_container();
	}
	void on(uint8_t* v) override
	{
		SG_DEBUG("writing uint8_t with value {}\n", *v);
		m_json_writer.Uint(*v);
		close_container();
	}
	void on(int16_t* v) override
	{
		SG_DEBUG("writing int16_t with value {}\n", *v);
		m_json_writer.Int(*v);
		close_container();
	}
	void on(uint16_t* v) override
	{
		SG_DEBUG("writing uint16_t with value {}\n", *v);
		m_json_writer.Uint(*v);
		close_container();
	}
	void on(int32_t* v) override
	{
		SG_DEBUG("writing int32_t with value {}\n", *v);
		m_json_writer.Int(*v);
		close_container();
	}
	void on(uint32_t* v) override
	{
		SG_DEBUG("writing uint32_t with value {}\n", *v);
		m_json_writer.Uint(*v);
		close_container();
	}
	void on(int64_t* v) override
	{
		SG_DEBUG("writing int64_t with value {}\n", *v);
		m_json_writer.Int64(*v);
		close_container();
	}
	void on(uint64_t* v) override
	{
		SG_DEBUG("writing uint64_t with value {}\n", *v);
		m_json_writer.Uint64(*v);
		close_container();
	}
	void on(float* v) override
	{
		SG_DEBUG("writing float with value {}\n", *v);
		m_json_writer.Double(*v);
		close_container();
	}
	void on(float64_t* v) override
	{
		SG_DEBUG("writing double with value {}\n", *v);
		m_json_writer.Double(*v);
		close_container();
	}
	...
	{
		SG_DEBUG("writing complex128_t with value ({}, {})\n", v->real(), v->imag());
		m_json_writer.StartArray();
	...
	{
		SG_DEBUG("writing std::string with value {}\n", v->c_str());
	...
	{
		SG_DEBUG("writing matrix of size: {} x {}\n", *rows, *cols);
		m_json_writer.StartArray();
		if (*cols == 0 || *rows == 0)
		{
			m_json_writer.EndArray();
	...
	{
		SG_DEBUG("writing vector of size: {}\n", *size);
		m_json_writer.StartArray();
		if (*size == 0)
			m_json_writer.EndArray();
		else
	...
	{
		SG_DEBUG("writing std::vector of size: {}\n", *size);
		m_json_writer.StartArray();
		if (*size == 0)
			m_json_writer.EndArray();
		else
