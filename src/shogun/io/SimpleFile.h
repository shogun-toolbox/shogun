...
template <class T> class CSimpleFile : public CSGObject
{
	public:
		...
		{
			io::unstable("CSimpleFile::CSimpleFile()");
			...
			if (status)
			{
				...
				if (num==0)
				{
					...
					if (cur_pos!=-1)
					{
						if (!fseek(file, 0, SEEK_END))
						{
							...
							{
								io::info("file of size {} bytes == {} entries detected\n", num,num/sizeof(T));
								num/=sizeof(T);
							}
							else
								seek_status=false;
				...
				if (num>0)
				{
					...
					if (target)
					{
						...
						if (!status)
							error("only {} of {} entries read. io error\n", (int64_t) num_read, num);
					}
					else
						error("failed to allocate memory while trying to read {} entries from file \"s\"\n", (int64_t) num, filename);
				}
				return target;
			}
			else
