...
namespace shogun
{
	 ...
	 * messages filter output depending on the loglevel, i.e. CRITICAL messages
	 * will print all MSG_CRITICAL TO MSG_EMERGENCY messages.
	 */
	enum EMessageType
	{
		MSG_GCDEBUG = 0,
	...
	/** The io functions can optionally prepend the function name or the line
	 * from where the print occurred.
	 */
...
#define SG_UNSTABLE(func, ...)                                                 \
	{                                                                          \
		SG_IO->message(                                                        \
		    MSG_WARN, _SRC_LOC,                                                \
		    __FILE__                                                           \
		    ":" func ": Unstable method!  Please report if it seems to "       \
...
#define SG_PRINT(...)                                                          \
	{                                                                          \
		SG_IO->message(MSG_MESSAGEONLY, _SRC_LOC, __VA_ARGS__);                \
	}
#define SG_NOTIMPLEMENTED                                                      \
	{                                                                          \
		SG_IO->not_implemented(_SRC_LOC);                                      \
	}
#define SG_GPL_ONLY                                                            \
	{                                                                          \
		SG_IO->gpl_only(_SRC_LOC);                                             \
	}
...
#define SG_ERROR(...) _clang_fail();
	 ...
	 * this class to be in the end written to the screen. Note that messages
	 * don't have to be written to stdout or stderr, but can be redirected to a
	 * file.
	 */
	class SGIO
	{
	public:
		 ...
		 */
		void
		init_default_logger(uint64_t queue_size = 128, uint64_t n_threads = 1);
		...
		template <typename... Args>
		void message(
		    EMessageType prio, const char* format, const Args&... args) const
		{
		...
		template <typename Exception, typename... Args>
		void
		error(EMessageType prio, const char* msg, const Args&... args) const
		{
	...
	void SGIO::error(
	    EMessageType prio, const SourceLocation& loc, const char* msg,
	    const Args&... args) const
	{
		static_assert(
