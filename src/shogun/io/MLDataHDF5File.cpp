 ...
 * Written (W) 2013 Zhengyang Liu(zhengyangl)
 */
...
#define MLDATA_URL_PREFIX "http://mldata.org/repository/data/download/"
...
CMLDataHDF5File::CMLDataHDF5File(char* data_name, const char* name) : CFile()
{
	get_boolean_type();
    ...
    CURL *curl;
    FILE *fp;
    char *mldata_url = (char*)malloc(strlen((char*)MLDATA_URL_PREFIX)+strlen(data_name)+1);
    strcat(mldata_url, (char*)MLDATA_URL_PREFIX);
    strcat(mldata_url, data_name);
    fname = (char*)malloc(strlen((char*)"/tmp/") + strlen(data_name) +1);
