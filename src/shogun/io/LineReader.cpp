...
bool LineReader::has_next()
{
	...
	{
		SG_ERROR("LineReader::has_next():: Class is not initialized\n");
		return false;
	}

	if (ferror(m_stream))
	{
		SG_ERROR("LineReader::has_next():: Error reading file\n");
		return false;
	}
...
int32_t LineReader::read(int32_t& bytes_to_skip)
{
	...
	while (1)
	{
		...
		if (ferror(m_stream))
		{
			SG_ERROR("LineReader::read(int32_t&):: Error reading file\n");
			return -1;
		}
	}
}
