...
CTreeMaxProduct::~CTreeMaxProduct()
{
	if (m_msg_order.empty() == false)
	{
		for (std::vector<MessageEdge*>::iterator it = m_msg_order.begin();
				it != m_msg_order.end(); ++it)
			delete *it;
	}
}
...
void CTreeMaxProduct::init()
{
	...
	// caculate lookup tables for forward messages
	...
	{
		if (m_msg_order[mi]->mtype == 1) // var_to_factor
		{
			// <var_id, msg_id>
			m_msg_map_var[m_msg_order[mi]->child] = mi;
			// collect incoming msgs for each fac_id
			//m_msgset_map_fac[m_msg_order[mi]->parent].insert(mi);
		}
		else // factor_to_var
		{
			// <fac_id, msg_id>
	...
	std::vector<bool>& is_root) const
{
	...
	CDisjointSet* dset = m_fg->get_disjoint_set();
	if (dset->get_connected() == false)
	{
		SG_UNREF(dset);
		SG_ERROR("%s::get_root_indicators(): run connect_components() first!\n", get_name());
	}

	int32_t card_size = m_fg->get_cardinalities().size();
	if (is_root.size() != (uint32_t)card_size)
		is_root.resize(card_size);

	std::fill(is_root.begin(), is_root.end(), 0);
	
	for (int32_t vi = 0; vi < card_size; vi++)
	{
	...
	std::vector<GraphNode*> node_stack;
	...
	for (uint32_t ni = 0; ni < is_root.size(); ni++)
	{
		if (is_root[ni] == true)
		{
			...
			node_stack.push_back(new GraphNode(ni, 1, -1));
		}
	}
	...
	{
		GraphNode* node = node_stack.back();
		node_stack.pop_back();

		if (node->node_type == 1) // child: factor -> parent: var
		{
			...
			for (const_iter mi = adj_factors.first; mi != adj_factors.second; ++mi) 
			{
				...
				order[eid--] = new MessageEdge(0, adj_factor_id, node->node_id);
				// add factor node to node_stack
				node_stack.push_back(new GraphNode(adj_factor_id, 0, node->node_id));
			}
		}
		else // child: var -> parent: factor
		{
			...
			for (int32_t vi = 0; vi < vars.size(); vi++)
			{
				...
				order[eid--] = new MessageEdge(1, vars[vi], node->node_id);
				// add variable node to node_stack
				node_stack.push_back(new GraphNode(vars[vi], 1, node->node_id));
			}
		}
...
{
	ASSERT(assignment.size() == m_fg->get_cardinalities().size());
...
void CTreeMaxProduct::bottom_up_pass()
{
	...
	for (uint32_t mi = 0; mi < m_msg_order.size(); ++mi)
	{
		...
		if (m_msg_order[mi]->mtype == 1) // var -> factor
		{
	...
	for (uint32_t ri = 0; ri < m_is_root.size(); ri++)
	{
		...
		for (std::set<uint32_t>::const_iterator cit = msgset_rt.begin();
			cit != msgset_rt.end(); cit++)
		{
			std::transform(m_fw_msgs[*cit].begin(), m_fw_msgs[*cit].end(),
				rmarg.begin(), 
				rmarg.begin(),
		...
		m_logZ += *std::max_element(rmarg.begin(), rmarg.end());
	}
...
void CTreeMaxProduct::top_down_pass()
{
	...
	for (int32_t mi = (int32_t)(m_msg_order.size()-1); mi >= 0; --mi)
	{
		SG_DEBUG("mi = %d, mtype: %d %d <- %d\n", mi, 
			m_msg_order[mi]->mtype, m_msg_order[mi]->child, m_msg_order[mi]->parent);
		if (m_msg_order[mi]->mtype == 0) // factor <- var
		{
			int32_t fac_id = m_msg_order[mi]->child;
			int32_t var_id = m_msg_order[mi]->parent;
