...
float64_t SOSVMHelper::primal_objective(const SGVector<float64_t>& w, const std::shared_ptr<StructuredModel>& model, float64_t lbda)
{
...
float64_t SOSVMHelper::dual_objective(const SGVector<float64_t>& w, float64_t aloss, float64_t lbda)
{
	return (-lbda/2 * linalg::dot(w, w) + aloss);
}

float64_t SOSVMHelper::average_loss(const SGVector<float64_t>& w, const std::shared_ptr<StructuredModel>& model, bool is_ub)
{
	float64_t loss = 0.0;
	auto labels = model->get_labels();
	int32_t N = labels->get_num_labels();
