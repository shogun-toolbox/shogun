...
CSequenceLabels::CSequenceLabels(SGVector< index_t > labels, index_t label_length,
		index_t num_labels, index_t num_states)
: CStructuredLabels(num_labels), m_num_states(num_states)
{
	...
	{
		add_vector_label(SGVector< index_t >(
			SGVector< index_t >::clone_vector(labels.vector+i, label_length),
			label_length));
	}
}
