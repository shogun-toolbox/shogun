...
typedef std::unordered_map<index_t, index_t> factor_counts_type;
...
void CFactorGraphModel::add_factor_type(CFactorType* ftype)
{
	...
	SGVector<index_t> w_map_cp = m_w_map.clone();
...
void CFactorGraphModel::del_factor_type(const index_t ftype_id)
{
	int w_dim = 0;
...
CFactorType* CFactorGraphModel::get_factor_type(const index_t ftype_id) const
{
	for (index_t fi = 0; fi < m_factor_types->get_num_elements(); ++fi)
	{
		CFactorType* ftype = dynamic_cast<CFactorType*>(m_factor_types->get_element(fi));
...
SGVector<index_t> CFactorGraphModel::get_global_params_mapping() const
{
	return m_w_map.clone();
}

SGVector<index_t> CFactorGraphModel::get_params_mapping(const index_t ftype_id)
{
	return m_w_map.find(ftype_id);
}

index_t CFactorGraphModel::get_dim() const
{
	return m_w_map.size();
}
    ...
    index_t feat_idx, CStructuredData* y)
{
	...
	SGVector<index_t> states = fg_states->get_data();
	...
	for (index_t fi = 0; fi < facs->get_num_elements(); ++fi)
	{
		...
		index_t id = ftype->get_type_id();
		SGVector<index_t> w_map = get_params_mapping(id);

		ASSERT(w_map.size() == ftype->get_w_dim());

		SGVector<float64_t> dat = fac->get_data();
		index_t dat_size = dat.size();
    ...
    SGVector<float64_t> w, index_t feat_idx, bool const training)
{
	...
	SGVector<index_t> states_gt = y_truth->get_data();
	...
	SGVector<index_t> states_star = y_star->get_data();
...
float64_t CFactorGraphModel::delta_loss(CStructuredData* y1, CStructuredData* y2)
{
	...
	SGVector<index_t> s_truth = y_truth->get_data();
	SGVector<index_t> s_pred = y_pred->get_data();
