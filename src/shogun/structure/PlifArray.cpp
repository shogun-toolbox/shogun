...
float64_t
CPlifArray::lookup_penalty(index_t p_value, float64_t* svm_values) const
{
	//min_value = -1e6 ;
	//max_value = 1e6 ;
	if (p_value<min_value || p_value>max_value)
...
index_t CPlifArray::get_max_id() const
{
	index_t max_id = 0;
	for (index_t i = 0; i < m_array.get_num_elements(); i++)
...
void CPlifArray::get_used_svms(int32_t* num_svms, index_t* svm_ids)
{
