    ...
    index_t N)
{
	...
	SGVector<index_t> test_var(N * N);
	assignment_expect = SGVector<index_t>(N * N);
...
CFactorGraph* CFactorGraphDataGenerator::multi_state_tree_graph()
{
	...
	SGVector<index_t>::fill_vector(vc.vector, vc.vlen, 3);
	CFactorGraph* fg = new CFactorGraph(vc);
	SG_REF(fg);
    ...
    SGMatrix<float64_t>& feats, SGMatrix<index_t>& labels)
{
	...
	labels = SGMatrix<index_t>(len_label, size_data);

	for (index_t k = 0; k < size_data; k++)
	{
		// generate a label vector
		SGVector<index_t> v_label(len_label);
		v_label.zero();
		index_t i = k % len_label;
		v_label[i] = 1;

		// generate feature vector
		SGVector<index_t> random_indices(len_feat);
    ...
    CFactorGraphFeatures* fg_feats, CFactorGraphLabels* fg_labels)
{
	...
	{
		SGVector<index_t> vc(num_classes);
		SGVector<index_t>::fill_vector(vc.vector, vc.vlen, 2);
		...
		{
			SGVector<index_t> var_index_u(1);
			var_index_u[0] = u;
		...
		for (index_t t = 0; t < num_edges; t++)
		{
			...
			data_t[0] = 1.0;
			SGVector<index_t> var_index_t = edge_list.get_row_vector(t);
		...
		// add label
		index_t* plabs = labels.get_column_vector(n);
		SGVector<index_t> states_gt(num_classes);
		sg_memcpy(states_gt.vector, plabs, num_classes * sizeof(index_t));
...
float64_t CFactorGraphDataGenerator::test_sosvm(EMAPInferType infer_type)
{
	...
	for (index_t i = 0; i < num_sample_train; ++i)
	{
		...
		SGVector<index_t> s_t = y_t->get_data();
		SGVector<index_t> s_p = y_p->get_data();
