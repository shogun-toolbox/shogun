...
CFGTestData::CFGTestData(): CSGObject()
{}

CFactorGraph* CFGTestData::simple_chain_graph()
{
	// ftype
...
int32_t CFGTestData::grid_to_index(int32_t x, int32_t y, int32_t w)
{
	return x + w * y;
}

void CFGTestData::truncate_energy(float64_t &A, float64_t &B, float64_t &C, float64_t &D)
{
	if (A + D > C + B)
	{
...
CFactorGraph* CFGTestData::random_chain_graph(SGVector<int> &assignment_expect, float64_t &min_energy_expect, int32_t N)
{
...
CFactorGraph* CFGTestData::multi_state_tree_graph()
{
	// ftype
	SGVector<int32_t> card(2);
	card[0] = 3;
...
void CFGTestData::generate_data(int32_t len_label, int32_t len_feat, int32_t size_data,
...
SGMatrix< int32_t > CFGTestData::get_edges_full(const int32_t num_classes)
{
...
void CFGTestData::build_factor_graph(SGMatrix<float64_t> feats, SGMatrix<int32_t> labels,
 ...
 */
void CFGTestData::define_factor_types(int32_t num_classes, int32_t dim, int32_t num_edges,
...
float64_t CFGTestData::test_sosvm(EMAPInferType infer_type)
{
	...
	generate_data(10, 40, 40, feats_train, labels_train);
