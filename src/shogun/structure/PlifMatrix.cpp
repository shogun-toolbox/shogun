...
void CPlifMatrix::set_plif_ids(SGVector<int32_t> plif_ids)
{
	if (plif_ids.vlen!=m_num_plifs)
		error("plif_ids size mismatch (num_ids={} vs.num_plifs={})\n", plif_ids.vlen, m_num_plifs);
...
void CPlifMatrix::set_plif_min_values(SGVector<float64_t> min_values)
{
	if (min_values.vlen!=m_num_plifs)
		error("plif_values size mismatch (num_values={} vs.num_plifs={})\n", min_values.vlen, m_num_plifs);
...
void CPlifMatrix::set_plif_limits(SGMatrix<float64_t> limits)
{
	...
	{
		error("limits size mismatch expected ({},{}) got ({},{})\n",
...
void CPlifMatrix::set_plif_penalties(SGMatrix<float64_t> penalties)
{
	...
	{
		error("penalties size mismatch expected ({},{}) got ({},{})\n",
...
void CPlifMatrix::set_plif_names(SGVector<char>* names, int32_t num_values, int32_t maxlen)
{
	if (num_values!=m_num_plifs)
		error("names size mismatch (num_values={} vs.num_plifs={})\n", num_values, m_num_plifs);
...
{
	if (num_values!=m_num_plifs)
		error("transform_type size mismatch (num_values={} vs.num_plifs={})\n", num_values, m_num_plifs);
	...
	for (int32_t i=0; i<m_num_plifs; i++)
	{
		...
		if (!m_PEN[id]->set_transform_type(transform_str))
		{
			...
			m_num_plifs=0;
			m_num_limits=0;
			error("transform type not recognized ('{}')\n", transform_str);
		}
...
{
	if (m!=m_num_plifs)
		error("plif_state_signal_matrix size does not match previous info {}!={}\n", m, m_num_plifs);

	/*if (m_seq.get_dim3() != max_num_signals)
		error("size(plif_state_signal_matrix,2) does not match with size(m_seq,3): {}!={}\nSorry, Soeren... interface changed\n", m_seq.get_dim3(), max_num_signals);
