...
{
	SG_UNSTABLE("MAPInference::MAPInference()", "\n");
	...
	: SGObject()
{
	...
	REQUIRE(fg != NULL, "%s::MAPInference(): fg cannot be NULL!\n", get_name());
	...
	switch(inference_method)
	{
		...
		case GEMP_LP:
			...
			break;
		case LOOPY_MAX_PROD:
			SG_ERROR("%s::MAPInference(): LoopyMaxProduct has not been implemented!\n",
				get_name());
			break;
		case LP_RELAXATION:
			SG_ERROR("%s::MAPInference(): LPRelaxation has not been implemented!\n",
				get_name());
			break;
		case TRWS_MAX_PROD:
			SG_ERROR("%s::MAPInference(): TRW-S has not been implemented!\n",
				get_name());
			break;
		default:
			SG_ERROR("%s::MAPInference(): unsupported inference method!\n",
				get_name());
			break;
	}
