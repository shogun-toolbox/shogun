...
public:
	...
	struct Parameter
	{
		...
		Parameter(const index_t max_iter = 1000,
...
public:
	 ...
	 */
	void max_in_subdimension(SGNDArray<float64_t> tar_arr, SGVector<index_t> &subset_inds, SGNDArray<float64_t> &max_res) const;
	 ...
	 */
	index_t find_intersection_index(SGVector<index_t> region_A, SGVector<index_t> region_B);
