...
{
	int32_t node_id;
	int32_t node_type; // 1 var, 0 factor
	int32_t parent; // where came from

	GraphNode(int32_t id, int32_t type, int32_t pa)
...
{
public:
	int32_t mtype; // 1 var_to_factor, 0 factor_to_var
	int32_t child;
	int32_t parent;

public:
	MessageEdge(int32_t type, int32_t ch, int32_t pa)
	...
	{
		return mtype == 1 ? child : parent;
	}

	inline int32_t get_factor_node()
	{
		return mtype == 1 ? parent : child;
	}
};
...
private:
	...
	std::tr1::unordered_map<uint32_t, uint32_t> m_msg_map_var;
	std::tr1::unordered_map<uint32_t, uint32_t> m_msg_map_fac;
	std::tr1::unordered_map<uint32_t, std::set<uint32_t> > m_msgset_map_var;
	//std::tr1::unordered_map<uint32_t, std::set<uint32_t> > m_msgset_map_fac;
