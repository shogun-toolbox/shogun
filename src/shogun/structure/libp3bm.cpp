		...
		CRiskFunction*  risk_function)
{
	...
	float64_t *A_1=NULL, /* *A_2=NULL,*/ *H_buff, **ICPs;
	...
	subgrad_t=(float64_t**)LIBBMRM_CALLOC(nThreads, sizeof(float64_t*));
	Rt=(float64_t*)LIBBMRM_CALLOC(nThreads, sizeof(float64_t));
	...
	C=(float64_t*)LIBBMRM_CALLOC(nThreads, sizeof(float64_t));
	S=(uint8_t*)LIBBMRM_CALLOC(nThreads, sizeof(uint8_t));
	info=(TMultipleCPinfo**)LIBBMRM_CALLOC(nThreads, sizeof(TMultipleCPinfo*));
	if (H==NULL || A==NULL || b==NULL || beta==NULL || subgrad_t==NULL || diag_H==NULL || I==NULL || ICPcounter==NULL ||
			ICPs==NULL || ACPs==NULL || cp_list==NULL || prevW==NULL || wt==NULL || Rt==NULL || C==NULL || S==NULL || info==NULL)
	{
		p3bmrm.exitflag=-2;
		goto cleanup;
	}
	...
	to=0;
	N=(uint32_t)round((float64_t)((CRiskData*)data)->m_nFeatures/(float64_t)nThreads);
	...
	if (beta_start==NULL || beta_good==NULL || b2==NULL || diag_H2==NULL || I_start==NULL || I_good==NULL || I2==NULL || H2==NULL)
	{
		p3bmrm.exitflag=-2;
		goto cleanup;
	}
	...
	if (verbose)
		SG_SPRINT("%4d: tim=%.3lf, Fp=%lf, Fd=%lf, R=%lf, K=%lf, CPmodels=%d\n",
				p3bmrm.nIter, tstop-tstart, p3bmrm.Fp, p3bmrm.Fd, R, K, nThreads);
	...
	while (p3bmrm.exitflag==0)
	{
		...
		{
			cp_ptr=CPList_head;
			for (cp_i=0; cp_i<nThreads; ++cp_i)  /* for all cutting planes */
			{
			...
			cp_ptr=CPList_head;
			for (cp_i=0; cp_i<p3bmrm.nCP+nThreads; ++cp_i)  /* for all cutting planes */
			{
				A_1=get_cutting_plane(cp_ptr);
				for (uint32_t p=0; p<nThreads; ++p)
				{
			...
			for (uint32_t i=0; i<p3bmrm.nCP; ++i)
				for (uint32_t j=0; j<nThreads; ++j)
					H[LIBBMRM_INDEX(i, p3bmrm.nCP+j, BufSize)]=H[LIBBMRM_INDEX(p3bmrm.nCP+j, i, BufSize)];
		}
		...
		for (uint32_t p=0; p<nThreads; ++p)
		{
			I[p3bmrm.nCP-nThreads+p]=p+1;
			beta[p3bmrm.nCP-nThreads+p]=0.0;
		}
		...
		/* Stopping conditions */
		if (p3bmrm.Fp - p3bmrm.Fd <= TolRel*LIBBMRM_ABS(p3bmrm.Fp)) p3bmrm.exitflag=1;
		if (p3bmrm.Fp - p3bmrm.Fd <= TolAbs) p3bmrm.exitflag=2;
		if (p3bmrm.nCP >= BufSize) p3bmrm.exitflag=-1;
		...
		if (verbose)
			SG_SPRINT("%4d: tim=%.3lf, Fp=%lf, Fd=%lf, (Fp-Fd)=%lf, (Fp-Fd)/Fp=%lf, R=%lf, nCP=%d, nzA=%d, wdist=%lf, alpha=%lf, qp_cnt=%d, gamma=%lf, tuneAlpha=%d\n",
					p3bmrm.nIter, tstop-tstart, p3bmrm.Fp, p3bmrm.Fd, p3bmrm.Fp-p3bmrm.Fd, (p3bmrm.Fp-p3bmrm.Fd)/p3bmrm.Fp, R, p3bmrm.nCP, p3bmrm.nzA, wdist, alpha, qp_cnt, gamma, tuneAlpha);
		...
		if (cleanICP)
		{
			...
			if (cntICP > 0)
			{
				...
				for (uint32_t i=0; i<cntICP; ++i)
				{
					...
					LIBBMRM_MEMMOVE(b+tmp_idx, b+tmp_idx+1, (p3bmrm.nCP+nThreads-tmp_idx)*sizeof(float64_t));
