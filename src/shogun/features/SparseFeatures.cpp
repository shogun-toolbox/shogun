...
template<class ST> CSparseFeatures<ST>::CSparseFeatures(index_t size)
...
template<class ST> ST CSparseFeatures<ST>::get_feature(index_t num, index_t index)
{
...
template<class ST> SGVector<ST> CSparseFeatures<ST>::get_full_feature_vector(index_t num)
{
	SGSparseVector<ST> sv=get_sparse_feature_vector(num);
...
template<class ST> index_t CSparseFeatures<ST>::get_nnz_features_for_vector(index_t num)
{
	SGSparseVector<ST> sv = get_sparse_feature_vector(num);
...
template<class ST> SGSparseVector<ST> CSparseFeatures<ST>::get_sparse_feature_vector(index_t num)
{
...
template<class ST> ST CSparseFeatures<ST>::dense_dot(ST alpha, index_t num, ST* vec, index_t dim, ST b)
{
	SGSparseVector<ST> sv=get_sparse_feature_vector(num);
...
template<class ST> void CSparseFeatures<ST>::add_to_dense_vec(float64_t alpha, index_t num, float64_t* vec, index_t dim, bool abs_val)
{
...
void CSparseFeatures<complex128_t>::add_to_dense_vec(float64_t alpha,
	index_t num, float64_t* vec, index_t dim, bool abs_val)
{
	SG_NOTIMPLEMENTED;
}

template<class ST> void CSparseFeatures<ST>::free_sparse_feature_vector(index_t num)
{
	if (feature_cache)
...
template<class ST> index_t  CSparseFeatures<ST>::get_num_vectors() const
{
	return m_subset_stack->has_subsets() ? m_subset_stack->get_size() : sparse_feature_matrix.num_vectors;
}

template<class ST> index_t  CSparseFeatures<ST>::get_num_features() const
{
	return sparse_feature_matrix.num_features;
}

template<class ST> index_t CSparseFeatures<ST>::set_num_features(index_t num)
{
...
template<class ST> void CSparseFeatures<ST>::free_feature_vector(index_t num)
{
	if (feature_cache)
		feature_cache->unlock_entry(m_subset_stack->subset_idx_conversion(num));
...
template<class ST> index_t CSparseFeatures<ST>::get_dim_feature_space() const
{
	return get_num_features();
}

template<class ST> float64_t CSparseFeatures<ST>::dot(index_t vec_idx1,
		CDotFeatures* df, index_t vec_idx2)
{
	ASSERT(df)
...
template<> float64_t CSparseFeatures<complex128_t>::dot(index_t vec_idx1,
		CDotFeatures* df, index_t vec_idx2)
{
	SG_NOTIMPLEMENTED;
	return 0.0;
...
template<class ST> float64_t CSparseFeatures<ST>::dense_dot(index_t vec_idx1, const float64_t* vec2, index_t vec2_len)
{
...
template<> float64_t CSparseFeatures<complex128_t>::dense_dot(index_t vec_idx1,
...
template<class ST> void* CSparseFeatures<ST>::get_feature_iterator(index_t vector_index)
{
...
template<class ST> bool CSparseFeatures<ST>::get_next_feature(index_t& index, float64_t& value, void* iterator)
{
...
template<> bool CSparseFeatures<complex128_t>::get_next_feature(index_t& index,
...
template<class ST> SGSparseVectorEntry<ST>* CSparseFeatures<ST>::compute_sparse_feature_vector(index_t num,
	index_t& len, SGSparseVectorEntry<ST>* target)
{
	SG_NOTIMPLEMENTED
