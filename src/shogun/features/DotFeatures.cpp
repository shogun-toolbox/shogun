...
#pragma omp parallel shared(num_threads, step)
	{
		...
		index_t t_start=thread_num*step;
		index_t t_stop=(thread_num==num_threads) ? stop : (thread_num+1)*step;
...
void CDotFeatures::dense_dot_range_subset(index_t* sub_index, index_t num, float64_t* output, float64_t* alphas, float64_t* vec, index_t dim, float64_t b)
{
	...
	#pragma omp parallel shared(num_threads, step)
	{
		...
		index_t t_start=thread_num*step;
		index_t t_stop=(thread_num==num_threads) ? num : (thread_num+1)*step;
...
SGMatrix<float64_t> CDotFeatures::get_cov(bool copy_data_for_speed)
{
	...
	else
	{
		...
		for (auto m = 0; m < dim - 1; m++)
		{
			for (auto n = m + 1; n < dim; n++)
			{
