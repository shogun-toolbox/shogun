...
CTOPFeatures::CTOPFeatures(
    index_t size, CHMM* p, CHMM* n, bool neglin, bool poslin)
...
bool CTOPFeatures::compute_relevant_indizes(CHMM* hmm, T_HMM_INDIZES* hmm_idx)
{
	...
	if (hmm_idx->num_p > 0)
	{
		hmm_idx->idx_p = SG_MALLOC(index_t, hmm_idx->num_p);
		ASSERT(hmm_idx->idx_p)
	}

	if (hmm_idx->num_q > 0)
	{
		hmm_idx->idx_q = SG_MALLOC(index_t, hmm_idx->num_q);
		ASSERT(hmm_idx->idx_q)
	}

	if (hmm_idx->num_a > 0)
	{
		hmm_idx->idx_a_rows = SG_MALLOC(index_t, hmm_idx->num_a);
		hmm_idx->idx_a_cols = SG_MALLOC(index_t, hmm_idx->num_a);
	...
	if (hmm_idx->num_b > 0)
	{
		hmm_idx->idx_b_rows = SG_MALLOC(index_t, hmm_idx->num_b);
		hmm_idx->idx_b_cols = SG_MALLOC(index_t, hmm_idx->num_b);
