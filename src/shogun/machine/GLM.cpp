...
GLM::GLM()
{
	SG_ADD(&distribution, "distribution_type", "variable to store name of distribution type",
			ParameterProperties::HYPER);
	SG_ADD(&m_eta, "eta", "threshold parameter that linearizes the exp() function above eta",
			ParameterProperties::HYPER);
	SG_ADD(&m_lambda, "lambda", "regularization parameter of penalty term", ParameterProperties::HYPER);
	SG_ADD(&m_alpha, "alpha", "weighting between L1 penalty and L2 penalty term of the loss function",
			ParameterProperties::HYPER);
	SG_ADD(&m_tolerance, "tolerance", "convergence threshold or stopping criteria",
			ParameterProperties::HYPER);
...
GLM::GLM(GLM_DISTRIBUTION distr, float64_t alpha, float64_t lambda, float64_t learning_rate,
			int32_t max_iterations, float64_t tolerance, float64_t eta): GLM()
{
	distribution=distr;
	m_alpha=alpha;
	m_lambda=lambda;
...
void GLM::iteration()
{
	...
	auto gradient_w = m_cost_function->get_gradient(X, y, m_w, bias, m_lambda, m_alpha, m_compute_bias,
													m_eta, distribution);
	auto gradient_bias = m_cost_function->get_gradient_bias(X, y, m_w, bias, m_compute_bias, m_eta, distribution);
