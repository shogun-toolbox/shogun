...
{
	if (num<3)
		SG_ERROR("Number of landmarks should be greater than 3 to make triangulation possible while {} given.",
		         num);
...
CDenseFeatures<float64_t>* CMultidimensionalScaling::embed_distance(CDistance* distance)
{
	...
	if (m_landmark)
	{
		...
		if (parameters.landmark_ratio > 1.0) {
			SG_WARNING("Number of landmarks ({}) exceeds number of feature vectors ({})",m_landmark_number,distance->get_num_vec_lhs());
