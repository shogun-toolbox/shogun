...
index_t CClusteringEvaluation::find_match_count(SGVector<index_t> l1, index_t m1, SGVector<index_t> l2, index_t m2)
{
...
index_t CClusteringEvaluation::find_mismatch_count(SGVector<index_t> l1, index_t m1, SGVector<index_t> l2, index_t m2)
{
...
void CClusteringEvaluation::best_map(CLabels* predicted, CLabels* ground_truth)
{
	...
	SGVector<index_t> predicted_ilabels=((CMulticlassLabels*) predicted)->get_int_labels();
	SGVector<index_t> groundtruth_ilabels=((CMulticlassLabels*) ground_truth)->get_int_labels();
	...
	for (index_t i=0; i < label_g.vlen; ++i)
	{
		...
		{
			G(i, j)=find_mismatch_count(groundtruth_ilabels, static_cast<index_t>(label_g[i]),
