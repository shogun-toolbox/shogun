...
bool Kernel::init(std::shared_ptr<Features> l, std::shared_ptr<Features> r)
{
	...
	REQUIRE(l, "Kernel::init(%p, %p): Left hand side features required!\n", l.get(), r.get())
	REQUIRE(r, "Kernel::init(%p, %p): Right hand side features required!\n", l.get(), r.get())
	...
	SG_DEBUG("leaving Kernel::init(%p, %p)\n", l.get(), r.get());
	return true;
}
...
std::shared_ptr<Kernel> Kernel::obtain_from_generic(std::shared_ptr<SGObject> kernel)
{
	if (kernel)
	{
		...
		REQUIRE(casted, "Kernel::obtain_from_generic(): Error, provided object"
				" of class \"%s\" is not a subclass of Kernel!\n",
				kernel->get_name());
		return casted;
	}
	else
